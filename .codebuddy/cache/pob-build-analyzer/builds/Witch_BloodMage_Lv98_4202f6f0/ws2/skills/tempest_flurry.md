...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 3.88% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 3.88% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.77% DPS，需要 +10% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 0.86% DPS，需要 +22% 达到 +20% DPS
...
**类别汇总**: Skill: +40.0 | Sim: +26.0 | Tree: +26.0 | Item: +16.0
...
**公式**: `× 0.7层 × effMult 0.6000  (持续 4.00s, 几率 4.1%)`

**类别汇总**: Skill: +2900.0 | Tree: +173.0
...
**公式**: `× effMult 1.2000  (持续 5.00s, 几率 7.3%)`
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 3.88%/% | 1 | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +17.4% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.77%/% | 0 | 需要 +10% 穿透 → DPS +18.6% |
| 3 | lightning_pen | BASE | +22.5% | % | 0.86%/% | 0 | 需要 +22% 穿透 → DPS +19.3% |
| 4 | fire_pen | BASE | +31.5% | % | 0.65%/% | 0 | 需要 +32% 穿透 → DPS +20.4% |
| 5 | crit_multi_base | BASE | +33.0% | % | 0.61%/% | 115 | CritBase 115→148, 需要 +33 → DPS +20.3% |
| 6 | attack_speed_inc | INC | +41.5% | % | 0.49%/% | 108 | INC 108%→150%, 需要 +42 → DPS +20.2% |
| 7 | crit_chance_inc | INC | +69.0% | % | 0.29%/% | 140 | INC 140%→209%, 需要 +69 → DPS +20.1% |
| 8 | flat_lightning_attack | BASE | +90.5 |  | 0.22%/1 | 0 | 需要添加 90-181 基础伤害 → DPS +19.8% |
| 9 | flat_physical_attack | BASE | +91.5 |  | 0.22%/1 | 0 | 需要添加 92-183 基础伤害 → DPS +20.2% |
| 10 | attack_damage_inc | INC | +92.0% | % | 0.22%/% | 183 | INC 183%→275%, 需要 +92 → DPS +20.0% |
| 11 | melee_damage_inc | INC | +92.0% | % | 0.22%/% | 183 | INC 183%→275%, 需要 +92 → DPS +20.0% |
| 12 | flat_fire_attack | BASE | +94.5 |  | 0.21%/1 | 0 | 需要添加 94-189 基础伤害 → DPS +19.8% |
| 13 | flat_cold_attack | BASE | +94.5 |  | 0.21%/1 | 0 | 需要添加 94-189 基础伤害 → DPS +19.8% |
| 14 | elemental_damage_inc | INC | +108.0% | % | 0.18%/% | 183 | INC 183%→291%, 需要 +108 → DPS +20.0% |
| 15 | crit_multi_inc | INC | +111.5% | % | 0.18%/% | 291 | INC 291%→402%, 需要 +112 → DPS +20.1% |
| 16 | lightning_damage_inc | INC | +237.0% | % | 0.08%/% | 183 | INC 183%→420%, 需要 +237 → DPS +20.0% |
| 17 | fire_damage_inc | INC | +308.0% | % | 0.07%/% | 183 | INC 183%→491%, 需要 +308 → DPS +20.0% |
...
| 2 | Stormcharged | Notable | -11.4% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
| 3 | Martial Artistry | Notable | -10.9% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
| 4 | Inspiring Ally | Notable | -10.4% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
...
| 6 | Crashing Wave | Notable | -7.8% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
| 7 | Critical Exploit | Notable | -7.3% | +0.0% | 输出 | 25% increased Critical Hit Chance |
| 8 | Heartstopping | Notable | -6.1% | -0.3% | 兼顾 | +10 to Intelligence; 20% increased Critical Hit Chance |
| 9 | Throatseeker | Notable | -5.9% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 10 | True Strike | Notable | -5.8% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 11 | All Natural | Notable | -5.6% | -4.4% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 12 | Material Solidification | Notable | -5.3% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 13 | For the Jugular | Notable | -4.8% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 14 | Heartbreaking | Notable | -4.5% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Strength |
| 15 | Struck Through | Notable | -4.4% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 16 | Whirling Assault | Notable | -3.8% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Dimensional Weakspot | Notable | +27.9% | +0.0% | 输出 |
| 2 | Forces of Nature | Notable | +27.9% | +0.0% | 输出 |
| 3 | Primal Sundering | Notable | +22.4% | +0.0% | 输出 |
| 4 | Electric Amplification | Notable | +21.9% | +0.0% | 输出 |
| 5 | Relinquish Your Life | Notable | +17.9% | +0.0% | 输出 |
| 6 | Cremation | Notable | +17.2% | +0.0% | 输出 |
| 7 | Storm Surge | Notable | +15.9% | +0.0% | 输出 |
| 8 | Exposed to the Storm | Notable | +15.8% | +0.0% | 输出 |
| 9 | Breath of Lightning | Notable | +13.2% | +0.0% | 输出 |
| 10 | Flash Storm | Notable | +13.2% | +0.0% | 输出 |
...
- **DPS 贡献**: +20.5% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +33.8% | +33.8% ⚠️模拟 | +0.0% | 30 |
...
| 2 | Herald of Thunder | +5.2% | +5.2% | +0.0% | 30 |
...
|------|--------|------|----------|-----------|
| # of Wind Dancer Stacks=0 | +0.0% | +0.0% | — | 108% |
| # of Wind Dancer Stacks=99999 | +0.0% | +0.0% | — | 108% |
...
- **构筑已有 modifier**：施法速度 INC 108%（来自 POB skillModList），总 MORE ×1.00。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +60.4% | +0.0% | 命中附带 Wither 叠层 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +15.2% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +10.8% | Low Life | 硬编码 |
...
当前 **Tempest Flurry** TotalDPS = **58,813**，AverageHit = 0，Speed = 4.08/s，CritChance = 48.8%，CritMultiplier = 5.50x。
...
| 3 | lightning_pen | BASE | 0 | +22% | 需要 +22% 穿透 → DPS +19.3% |
| 4 | fire_pen | BASE | 0 | +32% | 需要 +32% 穿透 → DPS +20.4% |
| 5 | crit_multi_base | BASE | 115 | +33% | CritBase 115→148, 需要 +33 → DPS +20.3% |
...
1. **Dimensional Weakspot**: DPS +27.9%
2. **Forces of Nature**: DPS +27.9%
3. **Primal Sundering**: DPS +22.4%
4. **Electric Amplification**: DPS +21.9%
5. **Relinquish Your Life**: DPS +17.9%
...
**最佳**: Heart of the Well (DPS +20.5%, EHP -0.0%)
