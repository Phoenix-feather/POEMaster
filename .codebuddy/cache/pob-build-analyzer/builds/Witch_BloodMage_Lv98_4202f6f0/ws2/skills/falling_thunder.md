...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 3.83% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 3.83% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.87% DPS，需要 +10% 达到 +20% DPS
3. **cold_pen** (BASE): 每 1% 提升 1.15% DPS，需要 +18% 达到 +20% DPS
...
**类别汇总**: Sim: +26.0 | Tree: +26.0 | Item: +16.0
...
**公式**: `× 0.8层 × effMult 0.6000  (持续 4.00s, 几率 13.5%)`
...
**公式**: `× 0.5层 × effMult 1.2000  (持续 5.00s, 几率 7.3%)`
...
### Combined DPS = 153,735

**类别汇总**: Hit: +127347.6 | DOT: +26387.0
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 3.83%/% | 1 | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +17.2% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.87%/% | 0 | 需要 +10% 穿透 → DPS +19.6% |
| 3 | cold_pen | BASE | +17.5% | % | 1.15%/% | 0 | 需要 +18% 穿透 → DPS +20.0% |
| 4 | crit_multi_base | BASE | +33.5% | % | 0.61%/% | 115 | CritBase 115→148, 需要 +34 → DPS +20.4% |
| 5 | attack_speed_inc | INC | +33.5% | % | 0.59%/% | 68 | INC 68%→102%, 需要 +34 → DPS +19.6% |
| 6 | lightning_pen | BASE | +38.5% | % | 0.51%/% | 0 | 需要 +38% 穿透 → DPS +19.6% |
| 7 | crit_chance_inc | INC | +70.0% | % | 0.29%/% | 140 | INC 140%→210%, 需要 +70 → DPS +20.1% |
| 8 | flat_physical_attack | BASE | +91.5 |  | 0.22%/1 | 0 | 需要添加 92-183 基础伤害 → DPS +20.1% |
| 9 | flat_lightning_attack | BASE | +92.5 |  | 0.21%/1 | 0 | 需要添加 92-185 基础伤害 → DPS +19.8% |
| 10 | flat_fire_attack | BASE | +95.5 |  | 0.21%/1 | 0 | 需要添加 96-191 基础伤害 → DPS +20.0% |
| 11 | flat_cold_attack | BASE | +95.5 |  | 0.21%/1 | 0 | 需要添加 96-191 基础伤害 → DPS +20.0% |
| 12 | attack_damage_inc | INC | +97.5% | % | 0.21%/% | 183 | INC 183%→280%, 需要 +98 → DPS +20.1% |
| 13 | melee_damage_inc | INC | +97.5% | % | 0.21%/% | 183 | INC 183%→280%, 需要 +98 → DPS +20.1% |
| 14 | elemental_damage_inc | INC | +102.0% | % | 0.20%/% | 183 | INC 183%→285%, 需要 +102 → DPS +20.1% |
| 15 | crit_multi_inc | INC | +114.0% | % | 0.18%/% | 291 | INC 291%→405%, 需要 +114 → DPS +20.1% |
| 16 | cold_damage_inc | INC | +176.5% | % | 0.11%/% | 183 | INC 183%→360%, 需要 +176 → DPS +20.1% |
| 17 | lightning_damage_inc | INC | +402.5% | % | 0.05%/% | 183 | INC 183%→586%, 需要 +402 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Eldritch Battery | Keystone | -14.1% | -8.9% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 2 | Stormcharged | Notable | -11.9% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
...
| 4 | Inspiring Ally | Notable | -9.9% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 5 | Deadly Force | Notable | -7.9% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
| 6 | Crashing Wave | Notable | -7.4% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
...
| 9 | All Natural | Notable | -5.9% | -4.4% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 10 | True Strike | Notable | -5.7% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 11 | Throatseeker | Notable | -5.7% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 12 | Whirling Assault | Notable | -4.8% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 13 | For the Jugular | Notable | -4.7% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Dimensional Weakspot | Notable | +29.4% | +0.0% | 输出 |
| 2 | Forces of Nature | Notable | +29.4% | +0.0% | 输出 |
| 3 | Glaciation | Notable | +24.5% | +0.0% | 输出 |
| 4 | Primal Sundering | Notable | +23.5% | +0.0% | 输出 |
| 5 | Storm Swell | Notable | +20.8% | +0.0% | 输出 |
| 6 | Exposed to the Cosmos | Notable | +20.1% | +0.0% | 输出 |
| 7 | Snowpiercer | Notable | +16.9% | +0.3% | 兼顾 |
| 8 | Breath of Ice | Notable | +16.9% | +0.3% | 兼顾 |
| 9 | Storm Surge | Notable | +16.6% | +0.0% | 输出 |
| 10 | Electric Amplification | Notable | +12.5% | +0.0% | 输出 |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +38.5% | +38.5% ⚠️模拟 | +0.0% | 30 |
...
| 2 | Herald of Thunder | +5.1% | +5.1% | +0.0% | 30 |
...
|------|--------|------|----------|-----------|
| # of Wind Dancer Stacks=0 | +0.0% | +0.0% | — | 68% |
| # of Wind Dancer Stacks=99999 | +0.0% | +0.0% | — | 68% |
...
- **构筑已有 modifier**：施法速度 INC 68%（来自 POB skillModList），总 MORE ×1.00。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +59.3% | +0.0% | 命中附带 Wither 叠层 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +14.4% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +10.3% | Low Life | 硬编码 |
...
当前 **Falling Thunder** TotalDPS = **127,348**，AverageHit = 0，Speed = 1.41/s，CritChance = 48.8%，CritMultiplier = 5.50x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +17.2% |
| 2 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +19.6% |
| 3 | cold_pen | BASE | 0 | +18% | 需要 +18% 穿透 → DPS +20.0% |
| 4 | crit_multi_base | BASE | 115 | +34% | CritBase 115→148, 需要 +34 → DPS +20.4% |
| 5 | attack_speed_inc | INC | 68 | +34% | INC 68%→102%, 需要 +34 → DPS +19.6% |
...
1. **Dimensional Weakspot**: DPS +29.4%
2. **Forces of Nature**: DPS +29.4%
3. **Glaciation**: DPS +24.5%
4. **Primal Sundering**: DPS +23.5%
5. **Storm Swell**: DPS +20.8%
