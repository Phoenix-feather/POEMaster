...
1. **crit_chance_base** (BASE): 每 1% 提升 3.77% DPS，需要 +6% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.34% DPS，需要 +14% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.19% DPS，需要 +18% 达到 +20% DPS
...
**公式**: `× 0.3层 × effMult 0.6000  (持续 4.00s, 几率 1.2%)`
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +5.5% | % | 3.77%/% | 1 | baseCrit 1.0%→6.5%, 需要 +5.5% → DPS +20.7% |
| 2 | elemental_pen | BASE | +14.5% | % | 1.34%/% | 0 | 需要 +14% 穿透 → DPS +19.4% |
| 3 | lightning_pen | BASE | +17.5% | % | 1.19%/% | 0 | 需要 +18% 穿透 → DPS +20.9% |
| 4 | attack_speed_inc | INC | +48.5% | % | 0.41%/% | 141 | INC 141%→190%, 需要 +48 → DPS +20.0% |
| 5 | attack_damage_inc | INC | +53.0% | % | 0.38%/% | 82 | INC 82%→135%, 需要 +53 → DPS +20.0% |
| 6 | melee_damage_inc | INC | +53.0% | % | 0.38%/% | 82 | INC 82%→135%, 需要 +53 → DPS +20.0% |
| 7 | flat_lightning_attack | BASE | +67.0 |  | 0.30%/1 | 0 | 需要添加 67-134 基础伤害 → DPS +20.0% |
| 8 | flat_fire_attack | BASE | +70.5 |  | 0.28%/1 | 0 | 需要添加 70-141 基础伤害 → DPS +20.0% |
| 9 | flat_cold_attack | BASE | +70.5 |  | 0.28%/1 | 0 | 需要添加 70-141 基础伤害 → DPS +20.0% |
| 10 | flat_physical_attack | BASE | +80.5 |  | 0.25%/1 | 0 | 需要添加 80-161 基础伤害 → DPS +19.8% |
| 11 | physical_damage_inc | INC | +100.0% | % | 0.20%/% | 82 | INC 82%→182%, 需要 +100 → DPS +20.0% |
| 12 | elemental_damage_inc | INC | +112.5% | % | 0.18%/% | 82 | INC 82%→194%, 需要 +112 → DPS +20.0% |
| 13 | lightning_damage_inc | INC | +136.0% | % | 0.15%/% | 82 | INC 82%→218%, 需要 +136 → DPS +20.0% |
| 14 | crit_chance_inc | INC | +788.0% | % | 0.03%/% | 36 | INC 36%→824%, 需要 +788 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Eldritch Battery | Keystone | -19.5% | -8.9% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 2 | Inspiring Ally | Notable | -18.0% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 3 | Chakra of Thought | Notable | -6.1% | +0.0% | 输出 | 8% of Damage is taken from Mana before Life; 15% increased Attack Speed while not on Low Mana |
| 4 | All Natural | Notable | -5.3% | -4.4% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 5 | Material Solidification | Notable | -4.1% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
...
| 8 | Struck Through | Notable | -3.5% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 9 | Whirling Assault | Notable | -3.3% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 10 | Falcon Dive | Notable | -3.3% | +0.0% | 输出 | 4% increased Attack Speed; 1% increased Attack Speed per 400 Accuracy Rating, up to 20% |
| 11 | Deep Trance | Notable | -3.3% | +0.0% | 输出 | 8% increased Attack Speed; 15% increased Cost Efficiency |
...
| 14 | Martial Artistry | Notable | -0.9% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Electric Amplification | Notable | +23.1% | +0.0% | 输出 |
| 2 | Comradery | Notable | +22.6% | +0.0% | 输出 |
| 3 | Exposed to the Storm | Notable | +20.9% | +0.0% | 输出 |
| 4 | Dimensional Weakspot | Notable | +20.8% | +0.0% | 输出 |
| 5 | Forces of Nature | Notable | +20.8% | +0.0% | 输出 |
| 6 | Breath of Lightning | Notable | +17.9% | +0.0% | 输出 |
| 7 | Surging Currents | Notable | +17.9% | +0.0% | 输出 |
| 8 | Flash Storm | Notable | +17.4% | +0.0% | 输出 |
| 9 | Storm Surge | Notable | +17.4% | +0.0% | 输出 |
| 10 | Overload | Notable | +17.4% | +0.0% | 输出 |
...
- **DPS 贡献**: +18.7% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +9.4% | -0.0% |
| DamageGainAsLightning | BASE | 13 | +9.4% | -0.0% |
| ManaOnKill | BASE | 1 | -0.0% | -0.0% |
| Condition:CanGainRage | FLAG | true | -0.0% | -0.0% |

### Megalomaniac (Diamond, UNIQUE)

- **DPS 贡献**: +3.3% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 61834
- **分配天赋**: the spring hare, blood of rage (DPS +3.3%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | the spring hare | +3.3% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +12.7% | +12.7% ⚠️模拟 | +0.0% | 30 |
...
| 2 | Herald of Thunder | +8.3% | +8.3% | +0.0% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +62.3% | +0.0% | 命中附带 Wither 叠层 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +26.4% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +18.9% | Low Life | 硬编码 |
...
当前 **Quarterstaff Strike** TotalDPS = **65,452**，AverageHit = 0，Speed = 5.27/s，CritChance = 1.7%，CritMultiplier = 2.95x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +6% | baseCrit 1.0%→6.5%, 需要 +5.5% → DPS +20.7% |
| 2 | elemental_pen | BASE | 0 | +14% | 需要 +14% 穿透 → DPS +19.4% |
| 3 | lightning_pen | BASE | 0 | +18% | 需要 +18% 穿透 → DPS +20.9% |
| 4 | attack_speed_inc | INC | 141 | +48% | INC 141%→190%, 需要 +48 → DPS +20.0% |
| 5 | attack_damage_inc | INC | 82 | +53% | INC 82%→135%, 需要 +53 → DPS +20.0% |
...
1. **Electric Amplification**: DPS +23.1%
2. **Comradery**: DPS +22.6%
3. **Exposed to the Storm**: DPS +20.9%
4. **Dimensional Weakspot**: DPS +20.8%
5. **Forces of Nature**: DPS +20.8%
...
**最佳**: Heart of the Well (DPS +18.7%, EHP -0.0%)
