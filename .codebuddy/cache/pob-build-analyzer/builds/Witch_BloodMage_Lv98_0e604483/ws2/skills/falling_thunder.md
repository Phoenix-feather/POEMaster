...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 5.82% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 5.82% DPS，需要 +4% 达到 +20% DPS
...
### Lightning Base Damage = 187-6589 (x7.47 base mult)
...
**公式**: `× effMult 0.6000  (持续 4.00s, 几率 45.6%)`
...
### Combined DPS = 391,084

**类别汇总**: Hit: +313135.4 | DOT: +77948.9
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占56.0%) | Enemy | +0.0 |
| 火焰 +0.0% (占28.0%) | Enemy | +0.0 |
| 冰霜 +0.0% (占14.0%) | Enemy | +0.0 |
| 物理 +0.0% (占2.0%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 5.82%/% | 1 | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +20.4% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.77%/% | 0 | 需要 +10% 穿透 → DPS +18.6% |
| 3 | lightning_pen | BASE | +19.0% | % | 1.06%/% | 0 | 需要 +19% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | +28.5% | % | 0.70%/% | 115 | CritBase 115→144, 需要 +28 → DPS +19.8% |
| 5 | fire_pen | BASE | +37.5% | % | 0.54%/% | 0 | 需要 +38% 穿透 → DPS +20.2% |
| 6 | attack_speed_inc | INC | +38.5% | % | 0.52%/% | 95 | INC 95%→134%, 需要 +38 → DPS +20.0% |
| 7 | crit_chance_inc | INC | +60.5% | % | 0.33%/% | 146 | INC 146%→206%, 需要 +60 → DPS +20.0% |
| 8 | attack_damage_inc | INC | +96.5% | % | 0.21%/% | 188 | INC 188%→284%, 需要 +96 → DPS +20.1% |
| 9 | melee_damage_inc | INC | +96.5% | % | 0.21%/% | 188 | INC 188%→284%, 需要 +96 → DPS +20.1% |
| 10 | elemental_damage_inc | INC | +110.0% | % | 0.18%/% | 188 | INC 188%→298%, 需要 +110 → DPS +20.0% |
| 11 | crit_multi_inc | INC | +110.5% | % | 0.18%/% | 351 | INC 351%→462%, 需要 +110 → DPS +20.0% |
| 12 | flat_lightning_attack | BASE | +120.0 |  | 0.17%/1 | 0 | 需要添加 120-240 基础伤害 → DPS +20.0% |
| 13 | flat_physical_attack | BASE | +124.5 |  | 0.16%/1 | 0 | 需要添加 124-249 基础伤害 → DPS +19.9% |
| 14 | flat_fire_attack | BASE | +125.0 |  | 0.16%/1 | 0 | 需要添加 125-250 基础伤害 → DPS +20.0% |
| 15 | flat_cold_attack | BASE | +125.0 |  | 0.16%/1 | 0 | 需要添加 125-250 基础伤害 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Stormcharged | Notable | -13.4% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 20% increased Critical Hit Chance |
| 2 | Eldritch Battery | Keystone | -10.9% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | Inspiring Ally | Notable | -10.0% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 4 | Deadly Force | Notable | -10.0% | +0.0% | 输出 | 30% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 12% increased Critical Hit Chance |
...
| 10 | All Natural | Notable | -5.5% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| 12 | Heartstopping | Notable | -5.1% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
| 13 | Struck Through | Notable | -5.1% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 14 | Throatseeker | Notable | -5.0% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 15 | For the Jugular | Notable | -4.6% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 16 | Heartbreaking | Notable | -4.5% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Strength |
...
- **DPS 贡献**: +20.1% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +39.4% | +41.6% ⚠️模拟 (辅助+2.1%) | +0.5% | 30 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +14.5% | Low Life | 硬编码 |
...
当前 **Falling Thunder** TotalDPS = **313,135**，AverageHit = 0，Speed = 1.64/s，CritChance = 64.2%，CritMultiplier = 7.74x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +20.4% |
| 2 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +18.6% |
| 3 | lightning_pen | BASE | 0 | +19% | 需要 +19% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | 115 | +28% | CritBase 115→144, 需要 +28 → DPS +19.8% |
...
**最佳**: Heart of the Well (DPS +20.1%, EHP -0.0%)
