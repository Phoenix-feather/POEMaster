...
2. **elemental_pen** (BASE): 每 1% 提升 1.80% DPS，需要 +12% 达到 +20% DPS
...
**公式**: `× 0.4层 × effMult 0.6000  (持续 4.00s, 几率 4.0%)`
...
| 2 | elemental_pen | BASE | +12.0% | % | 1.80%/% | 0 | 需要 +12% 穿透 → DPS +21.6% |
| 3 | lightning_pen | BASE | +20.5% | % | 0.97%/% | 0 | 需要 +20% 穿透 → DPS +19.9% |
| 4 | crit_multi_base | BASE | +30.0% | % | 0.69%/% | 115 | CritBase 115→145, 需要 +30 → DPS +20.6% |
| 5 | fire_pen | BASE | +37.5% | % | 0.54%/% | 0 | 需要 +38% 穿透 → DPS +20.3% |
| 6 | attack_speed_inc | INC | +38.0% | % | 0.54%/% | 86 | INC 86%→124%, 需要 +38 → DPS +20.4% |
| 7 | crit_chance_inc | INC | +63.5% | % | 0.32%/% | 146 | INC 146%→210%, 需要 +64 → DPS +20.2% |
| 8 | attack_damage_inc | INC | +91.0% | % | 0.22%/% | 188 | INC 188%→279%, 需要 +91 → DPS +20.1% |
| 9 | melee_damage_inc | INC | +91.0% | % | 0.22%/% | 188 | INC 188%→279%, 需要 +91 → DPS +20.1% |
| 10 | flat_physical_attack | BASE | +113.5 |  | 0.18%/1 | 0 | 需要添加 114-227 基础伤害 → DPS +20.0% |
| 11 | elemental_damage_inc | INC | +114.0% | % | 0.18%/% | 188 | INC 188%→302%, 需要 +114 → DPS +20.1% |
| 12 | crit_multi_inc | INC | +115.0% | % | 0.18%/% | 351 | INC 351%→466%, 需要 +115 → DPS +20.1% |
| 13 | flat_lightning_attack | BASE | +119.0 |  | 0.17%/1 | 0 | 需要添加 119-238 基础伤害 → DPS +20.2% |
| 14 | flat_fire_attack | BASE | +123.0 |  | 0.16%/1 | 0 | 需要添加 123-246 基础伤害 → DPS +20.0% |
| 15 | flat_cold_attack | BASE | +123.0 |  | 0.16%/1 | 0 | 需要添加 123-246 基础伤害 → DPS +20.0% |
| 16 | lightning_damage_inc | INC | +210.5% | % | 0.10%/% | 188 | INC 188%→398%, 需要 +210 → DPS +20.0% |
| 17 | fire_damage_inc | INC | +370.5% | % | 0.05%/% | 188 | INC 188%→558%, 需要 +370 → DPS +20.0% |
| 18 | physical_damage_inc | INC | +441.0% | % | 0.05%/% | 188 | INC 188%→629%, 需要 +441 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Stormcharged | Notable | -13.0% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 20% increased Critical Hit Chance |
| 2 | Eldritch Battery | Keystone | -11.1% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
...
| 8 | True Strike | Notable | -6.4% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 9 | Overflowing Power | Notable | -6.0% | +0.0% | 输出 | +2 to Maximum Power Charges |
| 10 | Material Solidification | Notable | -6.0% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 11 | All Natural | Notable | -5.3% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Forces of Nature | Notable | +27.0% | +0.0% | 输出 |
| 2 | Electric Amplification | Notable | +23.7% | +0.0% | 输出 |
| 3 | Primal Sundering | Notable | +21.6% | +0.0% | 输出 |
| 4 | Exposed to the Storm | Notable | +17.9% | +0.0% | 输出 |
| 5 | Storm Surge | Notable | +17.1% | +0.0% | 输出 |
| 6 | Cremation | Notable | +15.2% | +0.0% | 输出 |
| 7 | Surging Currents | Notable | +14.9% | +0.0% | 输出 |
| 8 | Breath of Lightning | Notable | +14.9% | +0.0% | 输出 |
| 9 | Flash Storm | Notable | +14.9% | +0.0% | 输出 |
| 10 | Overload | Notable | +14.9% | +0.0% | 输出 |
...
- **DPS 贡献**: +20.4% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +39.3% | +41.4% ⚠️模拟 (辅助+2.1%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +90.0% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +60.6% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Elemental Conflux（元素交融） | 60 | +18.0% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
...
当前 **Punch** TotalDPS = **42,981**，AverageHit = 0，Speed = 2.60/s，CritChance = 64.2%，CritMultiplier = 6.19x。
...
| 2 | elemental_pen | BASE | 0 | +12% | 需要 +12% 穿透 → DPS +21.6% |
| 3 | lightning_pen | BASE | 0 | +20% | 需要 +20% 穿透 → DPS +19.9% |
| 4 | crit_multi_base | BASE | 115 | +30% | CritBase 115→145, 需要 +30 → DPS +20.6% |
| 5 | fire_pen | BASE | 0 | +38% | 需要 +38% 穿透 → DPS +20.3% |
...
1. **Forces of Nature**: DPS +27.0%
2. **Electric Amplification**: DPS +23.7%
3. **Primal Sundering**: DPS +21.6%
4. **Exposed to the Storm**: DPS +17.9%
5. **Storm Surge**: DPS +17.1%
...
**最佳**: Heart of the Well (DPS +20.4%, EHP -0.0%)
