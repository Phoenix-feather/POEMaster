...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 4.30% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 4.30% DPS，需要 +4% 达到 +20% DPS
...
### Physical Base Damage = 147-589 (x5.08 base mult)
...
### Lightning Base Damage = 127-4481 (x5.08 base mult)
...
### Cold Base Damage = 671-1046 (x5.08 base mult)
...
### Fire Base Damage = 818-1300 (x5.08 base mult)
...
**公式**: `× effMult 0.6000  (持续 4.00s, 几率 25.2%)`
...
### Combined DPS = 326,213

**类别汇总**: Hit: +273106.0 | DOT: +53106.9
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占53.8%) | Enemy | +0.0 |
| 火焰 +0.0% (占28.9%) | Enemy | +0.0 |
| 冰霜 +0.0% (占14.4%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 4.30%/% | 1 | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +19.3% |
| 2 | elemental_pen | BASE | +12.0% | % | 1.80%/% | 0 | 需要 +12% 穿透 → DPS +21.6% |
| 3 | lightning_pen | BASE | +20.5% | % | 0.97%/% | 0 | 需要 +20% 穿透 → DPS +19.9% |
| 4 | crit_multi_base | BASE | +30.0% | % | 0.67%/% | 115 | CritBase 115→145, 需要 +30 → DPS +20.2% |
...
| 8 | attack_damage_inc | INC | +94.0% | % | 0.21%/% | 188 | INC 188%→282%, 需要 +94 → DPS +20.1% |
| 9 | melee_damage_inc | INC | +94.0% | % | 0.21%/% | 188 | INC 188%→282%, 需要 +94 → DPS +20.1% |
| 10 | elemental_damage_inc | INC | +114.0% | % | 0.18%/% | 188 | INC 188%→302%, 需要 +114 → DPS +20.1% |
| 11 | crit_multi_inc | INC | +116.5% | % | 0.17%/% | 351 | INC 351%→468%, 需要 +116 → DPS +20.0% |
| 12 | flat_lightning_attack | BASE | +119.0 |  | 0.17%/1 | 0 | 需要添加 119-238 基础伤害 → DPS +20.0% |
| 13 | flat_fire_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.0% |
| 14 | flat_cold_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.0% |
| 15 | flat_physical_attack | BASE | +130.5 |  | 0.15%/1 | 0 | 需要添加 130-261 基础伤害 → DPS +19.8% |
| 16 | lightning_damage_inc | INC | +210.5% | % | 0.10%/% | 188 | INC 188%→398%, 需要 +210 → DPS +20.0% |
| 17 | fire_damage_inc | INC | +371.0% | % | 0.05%/% | 188 | INC 188%→559%, 需要 +371 → DPS +20.0% |
...
| 6 | Crashing Wave | Notable | -7.7% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
| 7 | Martial Artistry | Notable | -7.7% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
...
| 10 | Material Solidification | Notable | -5.4% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 11 | All Natural | Notable | -5.3% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 12 | Heartstopping | Notable | -4.9% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
| 13 | Throatseeker | Notable | -4.8% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 14 | Struck Through | Notable | -4.8% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
...
| 6 | Cremation | Notable | +15.1% | +0.0% | 输出 |
...
- **DPS 贡献**: +20.3% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +39.0% | +41.1% ⚠️模拟 (辅助+2.1%) | +0.5% | 30 |
...
| 2 | Attrition（损耗） | 30 | +59.4% | +0.0% | 命中附带 Wither 叠层 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +15.0% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +10.7% | Low Life | 硬编码 |
...
当前 **Quarterstaff Strike** TotalDPS = **273,106**，AverageHit = 0，Speed = 2.60/s，CritChance = 64.2%，CritMultiplier = 6.19x。
...
| 4 | crit_multi_base | BASE | 115 | +30% | CritBase 115→145, 需要 +30 → DPS +20.2% |
...
**最佳**: Heart of the Well (DPS +20.3%, EHP -0.0%)
