...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 4.31% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 4.31% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.81% DPS，需要 +10% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.10% DPS，需要 +19% 达到 +20% DPS
...
### Physical Base Damage = 25-100 (x0.86 base mult)
...
### Lightning Base Damage = 22-759 (x0.86 base mult)
...
### Cold Base Damage = 114-177 (x0.86 base mult)
...
### Fire Base Damage = 138-220 (x0.86 base mult)
...
**类别汇总**: Aura: +27.0 | Sim: +26.0 | Tree: +26.0 | Item: +16.0
...
**公式**: `× 0.4层 × effMult 0.6500  (持续 4.00s, 几率 3.4%)`
...
**公式**: `× effMult 1.3000  (持续 5.00s, 几率 9.6%)`
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占56.5%) | Enemy | +0.0 |
| 火焰 +0.0% (占27.6%) | Enemy | +0.0 |
| 冰霜 +0.0% (占13.8%) | Enemy | +0.0 |
| 物理 +0.0% (占2.1%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 4.31%/% | 1 | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +19.4% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.81%/% | 0 | 需要 +10% 穿透 → DPS +19.0% |
| 3 | lightning_pen | BASE | +19.0% | % | 1.10%/% | 0 | 需要 +19% 穿透 → DPS +20.9% |
| 4 | crit_multi_base | BASE | +30.0% | % | 0.68%/% | 115 | CritBase 115→145, 需要 +30 → DPS +20.5% |
| 5 | fire_pen | BASE | +37.5% | % | 0.54%/% | 0 | 需要 +38% 穿透 → DPS +20.4% |
| 6 | attack_speed_inc | INC | +39.5% | % | 0.51%/% | 95 | INC 95%→134%, 需要 +40 → DPS +20.0% |
| 7 | crit_chance_inc | INC | +63.5% | % | 0.32%/% | 146 | INC 146%→210%, 需要 +64 → DPS +20.0% |
| 8 | attack_damage_inc | INC | +95.5% | % | 0.21%/% | 188 | INC 188%→284%, 需要 +96 → DPS +20.0% |
| 9 | elemental_damage_inc | INC | +107.5% | % | 0.19%/% | 188 | INC 188%→296%, 需要 +108 → DPS +20.0% |
| 10 | crit_multi_inc | INC | +115.0% | % | 0.17%/% | 351 | INC 351%→466%, 需要 +115 → DPS +20.0% |
| 11 | flat_lightning_attack | BASE | +119.0 |  | 0.17%/1 | 0 | 需要添加 119-238 基础伤害 → DPS +20.1% |
| 12 | flat_physical_attack | BASE | +119.5 |  | 0.17%/1 | 0 | 需要添加 120-239 基础伤害 → DPS +20.2% |
| 13 | flat_fire_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.1% |
| 14 | flat_cold_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.0% |
| 15 | lightning_damage_inc | INC | +191.0% | % | 0.10%/% | 188 | INC 188%→379%, 需要 +191 → DPS +20.0% |
| 16 | fire_damage_inc | INC | +370.0% | % | 0.05%/% | 188 | INC 188%→558%, 需要 +370 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Stormcharged | Notable | -13.3% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 20% increased Critical Hit Chance |
| 2 | Eldritch Battery | Keystone | -11.1% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | Inspiring Ally | Notable | -10.0% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 4 | Deadly Force | Notable | -9.8% | +0.0% | 输出 | 30% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 12% increased Critical Hit Chance |
...
| 7 | Crashing Wave | Notable | -7.5% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
...
| 11 | Throatseeker | Notable | -5.0% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 12 | Heartstopping | Notable | -4.9% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
| 13 | Struck Through | Notable | -4.8% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 14 | Material Solidification | Notable | -4.6% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
...
| 18 | The Power Within | Notable | -3.1% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Forces of Nature | Notable | +28.5% | +0.0% | 输出 |
| 2 | Electric Amplification | Notable | +26.0% | +0.0% | 输出 |
| 3 | Primal Sundering | Notable | +22.8% | +0.0% | 输出 |
| 4 | Exposed to the Storm | Notable | +19.8% | +0.0% | 输出 |
| 5 | Storm Surge | Notable | +18.6% | +0.0% | 输出 |
| 6 | Surging Currents | Notable | +16.5% | +0.0% | 输出 |
| 7 | Breath of Lightning | Notable | +16.5% | +0.0% | 输出 |
| 8 | Flash Storm | Notable | +16.5% | +0.0% | 输出 |
| 9 | Overload | Notable | +16.5% | +0.0% | 输出 |
| 10 | Cremation | Notable | +15.5% | +0.0% | 输出 |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +38.0% | +40.1% ⚠️模拟 (辅助+2.0%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +95.1% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +60.3% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Elemental Conflux（元素交融） | 60 | +19.0% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +14.7% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +10.5% | Low Life | 硬编码 |
...
当前 **Charged Shockwave** TotalDPS = **41,917**，AverageHit = 0，Speed = 2.73/s，CritChance = 64.2%，CritMultiplier = 6.19x。
...
| 2 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +19.0% |
| 3 | lightning_pen | BASE | 0 | +19% | 需要 +19% 穿透 → DPS +20.9% |
| 4 | crit_multi_base | BASE | 115 | +30% | CritBase 115→145, 需要 +30 → DPS +20.5% |
| 5 | fire_pen | BASE | 0 | +38% | 需要 +38% 穿透 → DPS +20.4% |
...
1. **Forces of Nature**: DPS +28.5%
2. **Electric Amplification**: DPS +26.0%
3. **Primal Sundering**: DPS +22.8%
4. **Exposed to the Storm**: DPS +19.8%
5. **Storm Surge**: DPS +18.6%
