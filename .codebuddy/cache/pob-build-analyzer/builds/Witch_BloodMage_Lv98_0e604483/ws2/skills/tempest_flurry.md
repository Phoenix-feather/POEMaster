...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 4.32% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 4.32% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.77% DPS，需要 +10% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.06% DPS，需要 +19% 达到 +20% DPS
...
### Physical Base Damage = 70-278 (x2.40 base mult)
...
### Lightning Base Damage = 60-2117 (x2.40 base mult)
...
### Cold Base Damage = 317-494 (x2.40 base mult)
...
### Fire Base Damage = 386-614 (x2.40 base mult)
...
### 通用伤害 MORE (Physical,Lightning,Cold,Fire) = -35.0%

**类别汇总**: Support: +-35%

| 来源 | 类别 | 值 |
|------|------|-----|
| Rapid Attacks III | Support | +-50.0% MORE |
| Rigwald's Ferocity | Support | +30.0% MORE |
...
**类别汇总**: Support: +40.0 | Aura: +27.0 | Sim: +26.0 | Tree: +26.0 | Item: +16.0

| 来源 | 类别 | 值 |
|------|------|-----|
| Rapid Attacks III | Support | +35.0 |
| Charge Infusion | Aura | +27.0 |
| ⚠ CI speed (模拟) | Sim | +26.0 |
| Hateforge, Moulded Mitts (Gloves) | Item | +16.0 |
| Rage III | Support | +15.0 |
| Rigwald's Ferocity | Support | -10.0 |
...
**公式**: `× effMult 0.6000  (持续 4.00s, 几率 6.2%)`

**类别汇总**: Support: +2900.0 | Tree: +178.0

| 来源 | 类别 | 值 |
|------|------|-----|
| 技能#SupportRigwaldsFerocityPlayer | Support | +2,900 |
| Crashing Wave | Tree | +36.0 |
| Deadly Force | Tree | +30.0 |
| Damage on Critical | Tree | +20.0 |
| Damage on Critical | Tree | +20.0 |
...
### Combined DPS = 134,341

**类别汇总**: Hit: +119585.6 | DOT: +14755.1
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占55.8%) | Enemy | +0.0 |
| 火焰 +0.0% (占27.8%) | Enemy | +0.0 |
| 冰霜 +0.0% (占13.9%) | Enemy | +0.0 |
| 物理 +0.0% (占2.5%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 4.32%/% | 1 | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +19.4% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.77%/% | 0 | 需要 +10% 穿透 → DPS +18.6% |
| 3 | lightning_pen | BASE | +19.0% | % | 1.06%/% | 0 | 需要 +19% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | +30.0% | % | 0.68%/% | 115 | CritBase 115→145, 需要 +30 → DPS +20.4% |
| 5 | fire_pen | BASE | +37.5% | % | 0.54%/% | 0 | 需要 +38% 穿透 → DPS +20.2% |
| 6 | attack_speed_inc | INC | +46.5% | % | 0.43%/% | 135 | INC 135%→182%, 需要 +46 → DPS +20.0% |
| 7 | crit_chance_inc | INC | +63.5% | % | 0.32%/% | 146 | INC 146%→210%, 需要 +64 → DPS +20.1% |
| 8 | attack_damage_inc | INC | +94.0% | % | 0.21%/% | 188 | INC 188%→282%, 需要 +94 → DPS +20.0% |
| 9 | melee_damage_inc | INC | +94.0% | % | 0.21%/% | 188 | INC 188%→282%, 需要 +94 → DPS +20.0% |
| 10 | elemental_damage_inc | INC | +110.0% | % | 0.18%/% | 188 | INC 188%→298%, 需要 +110 → DPS +20.0% |
| 11 | crit_multi_inc | INC | +116.0% | % | 0.17%/% | 351 | INC 351%→467%, 需要 +116 → DPS +20.1% |
| 12 | flat_physical_attack | BASE | +117.5 |  | 0.17%/1 | 0 | 需要添加 118-235 基础伤害 → DPS +20.2% |
| 13 | flat_lightning_attack | BASE | +119.0 |  | 0.17%/1 | 0 | 需要添加 119-238 基础伤害 → DPS +20.1% |
| 14 | flat_fire_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.0% |
| 15 | flat_cold_attack | BASE | +124.0 |  | 0.16%/1 | 0 | 需要添加 124-248 基础伤害 → DPS +20.0% |
| 16 | lightning_damage_inc | INC | +197.5% | % | 0.10%/% | 188 | INC 188%→386%, 需要 +198 → DPS +20.0% |
| 17 | fire_damage_inc | INC | +373.5% | % | 0.05%/% | 188 | INC 188%→562%, 需要 +374 → DPS +20.0% |

**无影响维度**: physical_damage_inc, cold_damage_inc, chaos_damage_inc, cold_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Stormcharged | Notable | -13.1% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 20% increased Critical Hit Chance |
| 2 | Eldritch Battery | Keystone | -11.0% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | Inspiring Ally | Notable | -10.2% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 4 | Deadly Force | Notable | -9.9% | +0.0% | 输出 | 30% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 12% increased Critical Hit Chance |
| 5 | Critical Exploit | Notable | -7.9% | +0.0% | 输出 | 25% increased Critical Hit Chance |
| 6 | Martial Artistry | Notable | -7.7% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
| 7 | Crashing Wave | Notable | -7.6% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
...
| 10 | Material Solidification | Notable | -5.8% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 11 | All Natural | Notable | -5.4% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 12 | Throatseeker | Notable | -4.9% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 13 | Heartstopping | Notable | -4.9% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
| 14 | Struck Through | Notable | -4.9% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 15 | For the Jugular | Notable | -4.6% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 16 | Heartbreaking | Notable | -4.4% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Strength |
| 17 | Whirling Assault | Notable | -3.4% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 18 | The Power Within | Notable | -3.0% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Forces of Nature | Notable | +27.9% | +0.0% | 输出 |
| 2 | Electric Amplification | Notable | +25.2% | +0.0% | 输出 |
| 3 | Primal Sundering | Notable | +22.3% | +0.0% | 输出 |
| 4 | Exposed to the Storm | Notable | +19.1% | +0.0% | 输出 |
| 5 | Storm Surge | Notable | +18.1% | +0.0% | 输出 |
| 6 | Surging Currents | Notable | +15.9% | +0.0% | 输出 |
| 7 | Breath of Lightning | Notable | +15.9% | +0.0% | 输出 |
| 8 | Flash Storm | Notable | +15.9% | +0.0% | 输出 |
| 9 | Overload | Notable | +15.9% | +0.0% | 输出 |
| 10 | Cremation | Notable | +15.3% | +0.0% | 输出 |
...
- **DPS 贡献**: +20.2% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +10.2% | -0.0% |
| DamageGainAsLightning | BASE | 13 | +10.0% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +34.5% | +36.4% ⚠️模拟 (辅助+1.9%) | +0.5% | 30 |
...
|------|--------|------|----------|-----------|
| # of Wind Dancer Stacks=0 | +0.0% | +0.0% | — | 135% |
| # of Wind Dancer Stacks=99999 | +0.0% | +0.0% | — | 135% |
...
- **构筑已有 modifier**：攻击速度 INC 135%（来自 POB skillModList），总 MORE ×0.65。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +92.9% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +60.1% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Elemental Conflux（元素交融） | 60 | +18.6% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
| 4 | Berserk（狂暴） | 30 | +17.3% | +0.0% | MORE Damage + 受伤增加 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +14.9% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +10.6% | Low Life | 硬编码 |
...
当前 **Tempest Flurry** TotalDPS = **119,586**，AverageHit = 0，Speed = 4.61/s，CritChance = 64.2%，CritMultiplier = 6.19x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→5.5%, 需要 +4.5% → DPS +19.4% |
| 2 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +18.6% |
| 3 | lightning_pen | BASE | 0 | +19% | 需要 +19% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | 115 | +30% | CritBase 115→145, 需要 +30 → DPS +20.4% |
| 5 | fire_pen | BASE | 0 | +38% | 需要 +38% 穿透 → DPS +20.2% |
...
1. **Forces of Nature**: DPS +27.9%
2. **Electric Amplification**: DPS +25.2%
3. **Primal Sundering**: DPS +22.3%
4. **Exposed to the Storm**: DPS +19.1%
5. **Storm Surge**: DPS +18.1%
...
**最佳**: Heart of the Well (DPS +20.2%, EHP -0.0%)
