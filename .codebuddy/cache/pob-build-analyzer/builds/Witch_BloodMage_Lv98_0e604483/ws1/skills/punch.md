...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 6.71% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 6.71% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.55% DPS，需要 +12% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.35% DPS，需要 +14% 达到 +20% DPS
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 6.71%/% | 1 | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +23.5% |
| 2 | elemental_pen | BASE | +12.5% | % | 1.55%/% | 0 | 需要 +12% 穿透 → DPS +19.4% |
| 3 | lightning_pen | BASE | +14.5% | % | 1.35%/% | 0 | 需要 +14% 穿透 → DPS +19.6% |
| 4 | attack_damage_inc | INC | +54.5% | % | 0.37%/% | 82 | INC 82%→136%, 需要 +54 → DPS +20.1% |
| 5 | melee_damage_inc | INC | +54.5% | % | 0.37%/% | 82 | INC 82%→136%, 需要 +54 → DPS +20.1% |
| 6 | attack_speed_inc | INC | +54.5% | % | 0.37%/% | 164 | INC 164%→218%, 需要 +54 → DPS +20.2% |
| 7 | flat_lightning_attack | BASE | +87.5 |  | 0.23%/1 | 0 | 需要添加 88-175 基础伤害 → DPS +20.0% |
| 8 | flat_fire_attack | BASE | +91.5 |  | 0.22%/1 | 0 | 需要添加 92-183 基础伤害 → DPS +20.0% |
| 9 | flat_cold_attack | BASE | +91.5 |  | 0.22%/1 | 0 | 需要添加 92-183 基础伤害 → DPS +20.1% |
| 10 | elemental_damage_inc | INC | +96.5% | % | 0.21%/% | 82 | INC 82%→178%, 需要 +96 → DPS +20.0% |
| 11 | flat_physical_attack | BASE | +104.0 |  | 0.19%/1 | 0 | 需要添加 104-208 基础伤害 → DPS +20.1% |
| 12 | lightning_damage_inc | INC | +113.0% | % | 0.18%/% | 82 | INC 82%→195%, 需要 +113 → DPS +20.0% |
| 13 | physical_damage_inc | INC | +120.0% | % | 0.17%/% | 82 | INC 82%→202%, 需要 +120 → DPS +20.0% |
| 14 | crit_chance_inc | INC | +446.5% | % | 0.04%/% | 31 | INC 31%→478%, 需要 +446 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Inspiring Ally | Notable | -17.1% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 2 | Eldritch Battery | Keystone | -15.0% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | Overflowing Power | Notable | -7.3% | +0.0% | 输出 | +2 to Maximum Power Charges |
| 4 | All Natural | Notable | -6.2% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 5 | Struck Through | Notable | -5.9% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
...
| 9 | The Power Within | Notable | -3.7% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
...
| 18 | Heartbreaking | Notable | -0.6% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Strength |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Electric Amplification | Notable | +28.3% | +0.0% | 输出 |
| 2 | Exposed to the Storm | Notable | +25.2% | +0.0% | 输出 |
| 3 | Forces of Nature | Notable | +24.2% | +0.0% | 输出 |
| 4 | Surging Currents | Notable | +21.0% | +0.0% | 输出 |
| 5 | Breath of Lightning | Notable | +21.0% | +0.0% | 输出 |
| 6 | Flash Storm | Notable | +21.0% | +0.0% | 输出 |
| 7 | Storm Surge | Notable | +21.0% | +0.0% | 输出 |
| 8 | Overload | Notable | +21.0% | +0.0% | 输出 |
| 9 | Primal Sundering | Notable | +19.4% | +0.0% | 输出 |
| 10 | Crushing Verdict | Notable | +16.2% | +0.0% | 输出 |
...
- **DPS 贡献**: +19.0% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +9.5% | -0.0% |
| DamageGainAsLightning | BASE | 13 | +9.5% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +12.0% | +12.5% ⚠️模拟 (辅助+0.5%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +80.8% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +65.5% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Berserk（狂暴） | 30 | +18.7% | +0.0% | MORE Damage + 受伤增加 |
| 4 | Elemental Conflux（元素交融） | 60 | +16.1% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
...
当前 **Punch** TotalDPS = **15,993**，AverageHit = 0，Speed = 5.85/s，CritChance = 2.1%，CritMultiplier = 3.55x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +23.5% |
| 2 | elemental_pen | BASE | 0 | +12% | 需要 +12% 穿透 → DPS +19.4% |
| 3 | lightning_pen | BASE | 0 | +14% | 需要 +14% 穿透 → DPS +19.6% |
| 4 | attack_damage_inc | INC | 82 | +54% | INC 82%→136%, 需要 +54 → DPS +20.1% |
| 5 | melee_damage_inc | INC | 82 | +54% | INC 82%→136%, 需要 +54 → DPS +20.1% |
...
1. **Electric Amplification**: DPS +28.3%
2. **Exposed to the Storm**: DPS +25.2%
3. **Forces of Nature**: DPS +24.2%
4. **Surging Currents**: DPS +21.0%
5. **Breath of Lightning**: DPS +21.0%
...
**最佳**: Heart of the Well (DPS +19.0%, EHP -0.0%)
