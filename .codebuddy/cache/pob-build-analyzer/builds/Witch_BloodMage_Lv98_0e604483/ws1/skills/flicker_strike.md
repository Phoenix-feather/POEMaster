...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 6.70% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 6.70% DPS，需要 +3% 达到 +20% DPS
2. **attack_speed_inc** (INC): 每 1% 提升 0.37% DPS，需要 +54% 达到 +20% DPS
3. **attack_damage_inc** (INC): 每 1% 提升 0.29% DPS，需要 +69% 达到 +20% DPS
...
### Lightning Base Damage = 110-5160 (x6.88 base mult)
...
**公式**: `× effMult 0.6000  (持续 4.00s, 几率 1.9%)`
...
**公式**: `× 0.8层 × effMult 1.2000  (持续 5.00s, 几率 0.3%)`
...
**类别汇总**: Hit: +3317140.5 | DOT: +22848.1

| 来源 | 类别 | 值 |
|------|------|-----|
| Hit DPS | Hit | +3,317,140 |
| 流血 DPS | DOT | +21,935 |
| 点燃 DPS | DOT | +913.5 |
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +200.0% (占71.0%) | Enemy | +200.0 |
| 物理 +0.0% (占20.8%) | Enemy | +0.0 |
| 火焰 +200.0% (占8.2%) | Enemy | +200.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.0% | % | 6.70%/% | 1 | baseCrit 1.0%→4.0%, 需要 +3.0% → DPS +20.1% |
| 2 | attack_speed_inc | INC | +54.5% | % | 0.37%/% | 164 | INC 164%→218%, 需要 +54 → DPS +20.2% |
| 3 | attack_damage_inc | INC | +69.0% | % | 0.29%/% | 82 | INC 82%→151%, 需要 +69 → DPS +20.0% |
| 4 | melee_damage_inc | INC | +69.0% | % | 0.29%/% | 82 | INC 82%→151%, 需要 +69 → DPS +20.0% |
| 5 | flat_lightning_attack | BASE | +86.0 |  | 0.23%/1 | 0 | 需要添加 86-172 基础伤害 → DPS +20.1% |
| 6 | elemental_damage_inc | INC | +86.5% | % | 0.23%/% | 82 | INC 82%→168%, 需要 +86 → DPS +20.1% |
| 7 | flat_fire_attack | BASE | +89.5 |  | 0.23%/1 | 0 | 需要添加 90-179 基础伤害 → DPS +20.2% |
| 8 | flat_cold_attack | BASE | +89.5 |  | 0.23%/1 | 0 | 需要添加 90-179 基础伤害 → DPS +20.2% |
| 9 | lightning_damage_inc | INC | +97.0% | % | 0.21%/% | 82 | INC 82%→179%, 需要 +97 → DPS +20.0% |
| 10 | flat_physical_attack | BASE | +133.5 |  | 0.15%/1 | 0 | 需要添加 134-267 基础伤害 → DPS +20.1% |
| 11 | physical_damage_inc | INC | +346.0% | % | 0.06%/% | 82 | INC 82%→428%, 需要 +346 → DPS +20.0% |
| 12 | crit_chance_inc | INC | +391.0% | % | 0.05%/% | 31 | INC 31%→422%, 需要 +391 → DPS +20.0% |

**无影响维度**: fire_damage_inc, cold_damage_inc, chaos_damage_inc, crit_multi_inc, crit_multi_base, lightning_pen, fire_pen, cold_pen, elemental_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Eldritch Battery | Keystone | -15.3% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 2 | Inspiring Ally | Notable | -13.9% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 3 | Overflowing Power | Notable | -7.4% | +0.0% | 输出 | +2 to Maximum Power Charges |
| 4 | All Natural | Notable | -7.0% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 5 | Struck Through | Notable | -6.7% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
...
| 7 | The Power Within | Notable | -3.7% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
| 8 | Acceleration | Notable | -3.7% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 9 | Flow Like Water | Notable | -3.1% | -0.2% | 兼顾 | 8% increased Attack and Cast Speed; +5 to Dexterity and Intelligence |
| 10 | Whirling Assault | Notable | -2.9% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 11 | Falcon Dive | Notable | -2.9% | +0.0% | 输出 | 1% increased Attack Speed per 250 Accuracy Rating |
...
| 14 | Material Solidification | Notable | -1.5% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | The Frenzied Bear | Notable | +12.7% | +0.4% | 兼顾 |
| 2 | Crushing Verdict | Notable | +12.4% | +0.0% | 输出 |
| 3 | Killer Instinct | Notable | +11.6% | +0.0% | 输出 |
| 4 | Wild Storm | Notable | +10.6% | +0.0% | 输出 |
| 5 | Jack of all Trades | Notable | +10.4% | +0.0% | 输出 |
| 6 | Imbibed Power | Notable | +9.6% | +0.0% | 输出 |
| 7 | Vile Wounds | Notable | +9.6% | +0.0% | 输出 |
| 8 | Overwhelm | Notable | +9.5% | +0.0% | 输出 |
| 9 | Singular Purpose | Notable | +9.5% | +0.0% | 输出 |
| 10 | Pure Power | Notable | +9.5% | +0.0% | 输出 |
...
- **DPS 贡献**: +16.2% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +8.1% | -0.0% |
| DamageGainAsLightning | BASE | 13 | +8.1% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +12.2% | +12.7% ⚠️模拟 (辅助+0.5%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +90.7% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +59.7% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Elemental Conflux（元素交融） | 60 | +18.1% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +20.3% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +14.5% | Low Life | 硬编码 |
...
当前 **Flicker Strike** TotalDPS = **3,317,140**，AverageHit = 0，Speed = 2.92/s，CritChance = 2.1%，CritMultiplier = 4.32x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +3% | baseCrit 1.0%→4.0%, 需要 +3.0% → DPS +20.1% |
| 2 | attack_speed_inc | INC | 164 | +54% | INC 164%→218%, 需要 +54 → DPS +20.2% |
| 3 | attack_damage_inc | INC | 82 | +69% | INC 82%→151%, 需要 +69 → DPS +20.0% |
| 4 | melee_damage_inc | INC | 82 | +69% | INC 82%→151%, 需要 +69 → DPS +20.0% |
| 5 | flat_lightning_attack | BASE | 0 | +86 | 需要添加 86-172 基础伤害 → DPS +20.1% |
...
1. **The Frenzied Bear**: DPS +12.7%，EHP +0.4%
2. **Crushing Verdict**: DPS +12.4%
3. **Killer Instinct**: DPS +11.6%
4. **Wild Storm**: DPS +10.6%
5. **Jack of all Trades**: DPS +10.4%
...
**最佳**: Heart of the Well (DPS +16.2%, EHP -0.0%)
