...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 6.79% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 6.79% DPS，需要 +3% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.80% DPS，需要 +12% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.54% DPS，需要 +12% 达到 +20% DPS
...
### Lightning Base Damage = 114-5332 (x7.11 base mult)
...
**公式**: `× 0.3层 × effMult 0.6000  (持续 4.00s, 几率 2.4%)`
...
### Combined DPS = 102,849

**类别汇总**: Hit: +100821.9 | DOT: +2026.9
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占86.2%) | Enemy | +0.0 |
| 火焰 +0.0% (占10.4%) | Enemy | +0.0 |
| 物理 +0.0% (占3.4%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.0% | % | 6.79%/% | 1 | baseCrit 1.0%→4.0%, 需要 +3.0% → DPS +20.4% |
| 2 | elemental_pen | BASE | +12.0% | % | 1.80%/% | 0 | 需要 +12% 穿透 → DPS +21.6% |
| 3 | lightning_pen | BASE | +12.5% | % | 1.54%/% | 0 | 需要 +12% 穿透 → DPS +19.3% |
| 4 | attack_speed_inc | INC | +57.0% | % | 0.36%/% | 173 | INC 173%→230%, 需要 +57 → DPS +20.3% |
| 5 | attack_damage_inc | INC | +67.5% | % | 0.30%/% | 82 | INC 82%→150%, 需要 +68 → DPS +20.1% |
| 6 | melee_damage_inc | INC | +67.5% | % | 0.30%/% | 82 | INC 82%→150%, 需要 +68 → DPS +20.1% |
| 7 | elemental_damage_inc | INC | +87.5% | % | 0.23%/% | 82 | INC 82%→170%, 需要 +88 → DPS +20.1% |
| 8 | lightning_damage_inc | INC | +98.5% | % | 0.20%/% | 82 | INC 82%→180%, 需要 +98 → DPS +20.1% |
| 9 | flat_lightning_attack | BASE | +99.0 |  | 0.20%/1 | 0 | 需要添加 99-198 基础伤害 → DPS +20.1% |
| 10 | flat_fire_attack | BASE | +104.0 |  | 0.19%/1 | 0 | 需要添加 104-208 基础伤害 → DPS +20.1% |
| 11 | flat_cold_attack | BASE | +104.0 |  | 0.19%/1 | 0 | 需要添加 104-208 基础伤害 → DPS +20.1% |
| 12 | flat_physical_attack | BASE | +105.5 |  | 0.19%/1 | 0 | 需要添加 106-211 基础伤害 → DPS +20.0% |
| 13 | physical_damage_inc | INC | +291.0% | % | 0.07%/% | 82 | INC 82%→373%, 需要 +291 → DPS +20.0% |
| 14 | crit_chance_inc | INC | +386.5% | % | 0.05%/% | 31 | INC 31%→418%, 需要 +386 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Inspiring Ally | Notable | -14.2% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 2 | Eldritch Battery | Keystone | -13.3% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | All Natural | Notable | -6.9% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 4 | Struck Through | Notable | -6.8% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 5 | Overflowing Power | Notable | -6.5% | +0.0% | 输出 | +2 to Maximum Power Charges |
| 6 | Chakra of Thought | Notable | -5.3% | +0.0% | 输出 | 8% of Damage is taken from Mana before Life; 15% increased Attack Speed while not on Low Mana |
| 7 | Material Solidification | Notable | -3.6% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 8 | Acceleration | Notable | -3.6% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 9 | The Power Within | Notable | -3.2% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
| 10 | Flow Like Water | Notable | -3.0% | -0.2% | 兼顾 | 8% increased Attack and Cast Speed; +5 to Dexterity and Intelligence |
| 11 | Whirling Assault | Notable | -2.8% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 12 | Falcon Dive | Notable | -2.8% | +0.0% | 输出 | 1% increased Attack Speed per 250 Accuracy Rating |
| 13 | Deep Trance | Notable | -2.8% | +0.0% | 输出 | 8% increased Attack Speed; 15% increased Cost Efficiency |
...
| 16 | Martial Artistry | Notable | -1.4% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
| 17 | Heartstopping | Notable | -1.0% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
| 18 | Heartbreaking | Notable | -0.7% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Strength |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Electric Amplification | Notable | +33.2% | +0.0% | 输出 |
| 2 | Exposed to the Storm | Notable | +28.9% | +0.0% | 输出 |
| 3 | Forces of Nature | Notable | +27.0% | +0.0% | 输出 |
| 4 | Surging Currents | Notable | +24.1% | +0.0% | 输出 |
| 5 | Breath of Lightning | Notable | +24.1% | +0.0% | 输出 |
| 6 | Flash Storm | Notable | +24.1% | +0.0% | 输出 |
| 7 | Storm Surge | Notable | +24.1% | +0.0% | 输出 |
| 8 | Overload | Notable | +24.1% | +0.0% | 输出 |
| 9 | Primal Sundering | Notable | +21.6% | +0.0% | 输出 |
| 10 | Storm Swell | Notable | +12.8% | +0.0% | 输出 |
...
| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +8.4% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +11.8% | +12.3% ⚠️模拟 (辅助+0.5%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +89.9% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
...
当前 **Falling Thunder** TotalDPS = **100,822**，AverageHit = 0，Speed = 3.62/s，CritChance = 2.1%，CritMultiplier = 4.32x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +3% | baseCrit 1.0%→4.0%, 需要 +3.0% → DPS +20.4% |
| 2 | elemental_pen | BASE | 0 | +12% | 需要 +12% 穿透 → DPS +21.6% |
| 3 | lightning_pen | BASE | 0 | +12% | 需要 +12% 穿透 → DPS +19.3% |
| 4 | attack_speed_inc | INC | 173 | +57% | INC 173%→230%, 需要 +57 → DPS +20.3% |
| 5 | attack_damage_inc | INC | 82 | +68% | INC 82%→150%, 需要 +68 → DPS +20.1% |
...
1. **Electric Amplification**: DPS +33.2%
2. **Exposed to the Storm**: DPS +28.9%
3. **Forces of Nature**: DPS +27.0%
4. **Surging Currents**: DPS +24.1%
5. **Breath of Lightning**: DPS +24.1%
