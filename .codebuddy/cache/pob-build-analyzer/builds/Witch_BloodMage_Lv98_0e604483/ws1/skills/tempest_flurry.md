...
2. **elemental_pen** (BASE): 每 1% 提升 1.79% DPS，需要 +10% 达到 +20% DPS
...
### Lightning Base Damage = 37-1718 (x2.29 base mult)
...
| Rigwald's Ferocity | Support | +-15.0% MORE |
...
**类别汇总**: Tree: +104.0 | Support: +80.0 | Aura: +27.0 | Sim: +26.0 | Item: +16.0

| 来源 | 类别 | 值 |
|------|------|-----|
| Rapid Attacks III | Support | +35.0 |
| Rigwald's Ferocity | Support | +30.0 |
...
**类别汇总**: Support: -1600.0 | Tree: +72.0

| 来源 | 类别 | 值 |
|------|------|-----|
| 技能#SupportRigwaldsFerocityPlayer | Support | -1,600 |
| Damage and Companion Damage | Tree | +12.0 |
| Damage and Companion Damage | Tree | +12.0 |
| Damage and Companion Damage | Tree | +12.0 |
| Damage and Companion Damage | Tree | +12.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 6.30%/% | 1 | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +22.1% |
| 2 | elemental_pen | BASE | +10.5% | % | 1.79%/% | 0 | 需要 +10% 穿透 → DPS +18.8% |
...
| 6 | attack_speed_inc | INC | +72.5% | % | 0.28%/% | 253 | INC 253%→326%, 需要 +72 → DPS +20.2% |
| 7 | elemental_damage_inc | INC | +83.5% | % | 0.24%/% | 82 | INC 82%→166%, 需要 +84 → DPS +20.1% |
| 8 | lightning_damage_inc | INC | +94.0% | % | 0.21%/% | 82 | INC 82%→176%, 需要 +94 → DPS +20.1% |
| 9 | flat_lightning_attack | BASE | +94.5 |  | 0.21%/1 | 0 | 需要添加 94-189 基础伤害 → DPS +19.9% |
| 10 | flat_fire_attack | BASE | +100.0 |  | 0.20%/1 | 0 | 需要添加 100-200 基础伤害 → DPS +20.2% |
| 11 | flat_cold_attack | BASE | +100.0 |  | 0.20%/1 | 0 | 需要添加 100-200 基础伤害 → DPS +20.2% |
| 12 | flat_physical_attack | BASE | +105.5 |  | 0.19%/1 | 0 | 需要添加 106-211 基础伤害 → DPS +20.0% |
| 13 | physical_damage_inc | INC | +302.5% | % | 0.07%/% | 82 | INC 82%→384%, 需要 +302 → DPS +20.0% |
...
| 2 | Eldritch Battery | Keystone | -13.8% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
...
| 12 | Falcon Dive | Notable | -2.2% | +0.0% | 输出 | 1% increased Attack Speed per 250 Accuracy Rating |
| 13 | Deep Trance | Notable | -2.2% | +0.0% | 输出 | 8% increased Attack Speed; 15% increased Cost Efficiency |
...
| 4 | Surging Currents | Notable | +25.2% | +0.0% | 输出 |
| 5 | Breath of Lightning | Notable | +25.2% | +0.0% | 输出 |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +9.0% | +9.4% ⚠️模拟 (辅助+0.4%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +94.2% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
...
当前 **Tempest Flurry** TotalDPS = **31,451**，AverageHit = 0，Speed = 10.87/s，CritChance = 2.1%，CritMultiplier = 3.55x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +22.1% |
| 2 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +18.8% |
...
4. **Surging Currents**: DPS +25.2%
5. **Breath of Lightning**: DPS +25.2%
