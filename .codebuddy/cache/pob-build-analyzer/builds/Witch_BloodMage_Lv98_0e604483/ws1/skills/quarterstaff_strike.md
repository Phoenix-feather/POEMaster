...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 6.16% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 6.16% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.53% DPS，需要 +14% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 1.37% DPS，需要 +16% 达到 +20% DPS
...
### Physical Base Damage = 1534-2062 (x4.84 base mult)
...
### Lightning Base Damage = 77-3630 (x4.84 base mult)
...
### Fire Base Damage = 53-106 (x4.84 base mult)
...
**公式**: `× 0.4层 × effMult 0.6000  (持续 4.00s, 几率 1.6%)`
...
### Combined DPS = 103,676

**类别汇总**: Hit: +102096.0 | DOT: +1580.1
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占79.3%) | Enemy | +0.0 |
| 火焰 +0.0% (占12.3%) | Enemy | +0.0 |
| 物理 +0.0% (占8.4%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 6.16%/% | 1 | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +21.6% |
| 2 | elemental_pen | BASE | +14.0% | % | 1.53%/% | 0 | 需要 +14% 穿透 → DPS +21.4% |
| 3 | lightning_pen | BASE | +15.5% | % | 1.37%/% | 0 | 需要 +16% 穿透 → DPS +21.2% |
| 4 | attack_speed_inc | INC | +54.5% | % | 0.37%/% | 164 | INC 164%→218%, 需要 +54 → DPS +20.2% |
| 5 | attack_damage_inc | INC | +58.0% | % | 0.35%/% | 82 | INC 82%→140%, 需要 +58 → DPS +20.1% |
| 6 | melee_damage_inc | INC | +58.0% | % | 0.35%/% | 82 | INC 82%→140%, 需要 +58 → DPS +20.1% |
| 7 | flat_lightning_attack | BASE | +92.5 |  | 0.22%/1 | 0 | 需要添加 92-185 基础伤害 → DPS +20.0% |
| 8 | flat_fire_attack | BASE | +96.5 |  | 0.21%/1 | 0 | 需要添加 96-193 基础伤害 → DPS +19.9% |
| 9 | flat_cold_attack | BASE | +96.5 |  | 0.21%/1 | 0 | 需要添加 96-193 基础伤害 → DPS +19.9% |
| 10 | elemental_damage_inc | INC | +102.0% | % | 0.20%/% | 82 | INC 82%→184%, 需要 +102 → DPS +20.0% |
| 11 | flat_physical_attack | BASE | +111.0 |  | 0.18%/1 | 0 | 需要添加 111-222 基础伤害 → DPS +20.0% |
| 12 | lightning_damage_inc | INC | +119.0% | % | 0.17%/% | 82 | INC 82%→201%, 需要 +119 → DPS +20.0% |
| 13 | physical_damage_inc | INC | +132.5% | % | 0.15%/% | 82 | INC 82%→214%, 需要 +132 → DPS +20.1% |
| 14 | crit_chance_inc | INC | +486.5% | % | 0.04%/% | 31 | INC 31%→518%, 需要 +486 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Inspiring Ally | Notable | -16.6% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 2 | Eldritch Battery | Keystone | -14.2% | -5.0% | 兼顾 | Converts all Energy Shield to Mana; Doubles Mana Costs |
| 3 | Overflowing Power | Notable | -6.9% | +0.0% | 输出 | +2 to Maximum Power Charges |
| 4 | All Natural | Notable | -5.9% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 5 | Chakra of Thought | Notable | -5.5% | +0.0% | 输出 | 8% of Damage is taken from Mana before Life; 15% increased Attack Speed while not on Low Mana |
| 6 | Struck Through | Notable | -5.4% | +0.0% | 输出 | Attacks have +1% to Critical Hit Chance |
| 7 | Material Solidification | Notable | -4.0% | +0.0% | 输出 | Gain 8% of Damage as Extra Physical Damage; 15% increased effect of Fully Broken Armour |
| 8 | Acceleration | Notable | -3.7% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 9 | The Power Within | Notable | -3.4% | +0.0% | 输出 | 20% increased Critical Damage Bonus if you've gained a Power Charge Recently; +1 to Maximum Power Charges |
| 10 | Flow Like Water | Notable | -3.1% | -0.2% | 兼顾 | 8% increased Attack and Cast Speed; +5 to Dexterity and Intelligence |
| 11 | Whirling Assault | Notable | -2.9% | +0.0% | 输出 | 8% increased Attack Speed with Quarterstaves; Knocks Back Enemies if you get a Critical Hit with a Quarterstaff |
| 12 | Falcon Dive | Notable | -2.9% | +0.0% | 输出 | 1% increased Attack Speed per 250 Accuracy Rating |
| 13 | Deep Trance | Notable | -2.9% | +0.0% | 输出 | 8% increased Attack Speed; 15% increased Cost Efficiency |
| 14 | Flow State | Notable | -1.8% | +0.0% | 输出 | 5% increased Skill Speed; 15% increased Mana Regeneration Rate |
...
| 16 | Martial Artistry | Notable | -1.3% | +0.0% | 输出 | 25% increased Accuracy Rating with Quarterstaves; 25% increased Critical Damage Bonus with Quarterstaves; +25 to Dexterity |
| 17 | Heartstopping | Notable | -0.9% | -0.3% | 兼顾 | +10 to Intelligence; 15% increased Critical Hit Chance |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Electric Amplification | Notable | +26.9% | +0.0% | 输出 |
| 2 | Exposed to the Storm | Notable | +23.9% | +0.0% | 输出 |
| 3 | Forces of Nature | Notable | +23.0% | +0.0% | 输出 |
| 4 | Surging Currents | Notable | +19.9% | +0.0% | 输出 |
| 5 | Breath of Lightning | Notable | +19.9% | +0.0% | 输出 |
| 6 | Flash Storm | Notable | +19.9% | +0.0% | 输出 |
| 7 | Storm Surge | Notable | +19.9% | +0.0% | 输出 |
| 8 | Overload | Notable | +19.9% | +0.0% | 输出 |
| 9 | Primal Sundering | Notable | +18.4% | +0.0% | 输出 |
| 10 | Crushing Verdict | Notable | +15.2% | +0.0% | 输出 |
...
- **DPS 贡献**: +17.9% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsFire | BASE | 14 | +9.0% | -0.0% |
| DamageGainAsLightning | BASE | 13 | +9.0% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Charge Infusion | +11.9% | +12.4% ⚠️模拟 (辅助+0.5%) | +0.5% | 30 |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +76.6% | +0.0% | 需精魄 100（缺 59）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +61.4% | +0.0% | 命中附带 Wither 叠层 |
| 3 | Berserk（狂暴） | 30 | +17.7% | +0.0% | MORE Damage + 受伤增加 |
| 4 | Elemental Conflux（元素交融） | 60 | +15.3% | +0.0% | 需精魄 60（缺 19）; 元素异常状态同步 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +24.3% | Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +17.3% | Low Life | 硬编码 |
...
当前 **Quarterstaff Strike** TotalDPS = **102,096**，AverageHit = 0，Speed = 5.85/s，CritChance = 2.1%，CritMultiplier = 3.55x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 1 | +4% | baseCrit 1.0%→4.5%, 需要 +3.5% → DPS +21.6% |
| 2 | elemental_pen | BASE | 0 | +14% | 需要 +14% 穿透 → DPS +21.4% |
| 3 | lightning_pen | BASE | 0 | +16% | 需要 +16% 穿透 → DPS +21.2% |
| 4 | attack_speed_inc | INC | 164 | +54% | INC 164%→218%, 需要 +54 → DPS +20.2% |
| 5 | attack_damage_inc | INC | 82 | +58% | INC 82%→140%, 需要 +58 → DPS +20.1% |
...
1. **Electric Amplification**: DPS +26.9%
2. **Exposed to the Storm**: DPS +23.9%
3. **Forces of Nature**: DPS +23.0%
4. **Surging Currents**: DPS +19.9%
5. **Breath of Lightning**: DPS +19.9%
...
**最佳**: Heart of the Well (DPS +17.9%, EHP -0.0%)
