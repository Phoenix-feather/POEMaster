...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 6.17% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 6.17% DPS，需要 +4% 达到 +20% DPS
2. **crit_multi_base** (BASE): 每 1% 提升 0.60% DPS，需要 +34% 达到 +20% DPS
3. **spell_damage_inc** (INC): 每 1% 提升 0.21% DPS，需要 +95% 达到 +20% DPS
...
### 通用伤害 INC (Lightning,Cold,Fire) = 267%

**类别汇总**: Tree: +227.0 | Skill: +40.0
...
### CritChance BASE = 48.6% (base 11.0% + added 0%)
...
**公式**: `基础 11053-16524 × 0.2层 × effMult 1.6770  (持续 4.40s, 几率 18.8%)`

**类别汇总**: Skill: -3100.0 | Sim: +2900.0 | Tree: +141.0
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 6.17%/% | 0 | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +21.6% |
| 2 | crit_multi_base | BASE | +33.5% | % | 0.60%/% | 100 | CritBase 100→134, 需要 +34 → DPS +20.2% |
| 3 | spell_damage_inc | INC | +95.0% | % | 0.21%/% | 378 | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 4 | elemental_damage_inc | INC | +95.0% | % | 0.21%/% | 378 | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 5 | crit_multi_inc | INC | +102.0% | % | 0.20%/% | 200 | INC 200%→302%, 需要 +102 → DPS +20.2% |
| 6 | crit_chance_inc | INC | +118.5% | % | 0.17%/% | 251 | INC 251%→370%, 需要 +118 → DPS +20.0% |
| 7 | cold_damage_inc | INC | +169.0% | % | 0.12%/% | 378 | INC 378%→547%, 需要 +169 → DPS +20.0% |
| 8 | fire_damage_inc | INC | +308.5% | % | 0.06%/% | 368 | INC 368%→676%, 需要 +308 → DPS +20.0% |

**无影响维度**: physical_damage_inc, lightning_damage_inc, chaos_damage_inc, speed_inc, lightning_pen, fire_pen, cold_pen, elemental_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Thin Ice | Notable | -10.6% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 2 | Throatseeker | Notable | -9.2% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 3 | Dynamism | Notable | -8.4% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 4 | Crashing Wave | Notable | -7.6% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
| 5 | Deadly Force | Notable | -6.9% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
| 6 | Potent Incantation | Notable | -6.3% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
| 7 | All Natural | Notable | -6.3% | +0.0% | 输出 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 8 | Careful Assassin | Notable | -5.1% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 9 | For the Jugular | Notable | -4.9% | -0.5% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 10 | I am the Blizzard... | Notable | -4.9% | +0.0% | 输出 | Gain 10% of Damage as Extra Cold Damage; On Freezing Enemies create Chilled Ground |
| 11 | I am the Thunder... | Notable | -4.8% | +0.0% | 输出 | Gain 10% of Damage as Extra Lightning Damage; 25% chance on Shocking Enemies to created Shocked Ground |
| 12 | Breaking Point | Notable | -4.7% | +0.0% | 输出 | 10% increased Duration of Elemental Ailments on Enemies; 30% increased Magnitude of Non-Damaging Ailments you inflict |
| 13 | Critical Exploit | Notable | -4.2% | +0.0% | 输出 | 25% increased Critical Hit Chance |
| 14 | Evocational Practitioner | Notable | -4.2% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
| 15 | True Strike | Notable | -3.4% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 16 | Sudden Escalation | Notable | -2.7% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 17 | Moment of Truth | Notable | -2.5% | +0.0% | 输出 | 25% increased Critical Damage Bonus if you've dealt a Non-Critical Hit Recently; 15% increased Critical Hit Chance |
...
| 天赋 | 移除后 EHP% | 效果 |
|------|-------------|------|
| Flow Like Water | -0.2% | 8% increased Attack and Cast Speed; +5 to Dexterity and Intelligence |
...
Invocated Echoes, ...and I Shall Rage, Impending Doom, Blood Transfusion, Energise, Heavy Frost, Marked Agility, Shimmering, Efficient Inscriptions, The Power Within, Overflowing Power, Infusion of Power, Marked for Sickness, Acceleration, Stormwalker, Frostwalker, The Soul Springs Eternal

## 7. 未分配天赋探索

| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Stormbreaker | Notable | +16.9% | +0.0% | 输出 |
| 2 | Harness the Elements | Notable | +16.9% | +0.0% | 输出 |
| 3 | Endless Blizzard | Notable | +13.9% | +0.0% | 输出 |
| 4 | Temporal Mastery | Notable | +13.3% | +0.0% | 输出 |
| 5 | Climate Change | Notable | +12.1% | +0.0% | 输出 |
| 6 | Cooked | Notable | +11.9% | -4.2% | 兼顾 |
| 7 | Limitless Pursuit | Notable | +11.7% | +0.0% | 输出 |
| 8 | Stormcharged | Notable | +11.2% | +0.0% | 输出 |
| 9 | Power of the Storm | Notable | +10.6% | +0.0% | 输出 |
| 10 | Multitasking | Notable | +10.0% | +0.0% | 输出 |
...
- **DPS 贡献**: +3.2% | **EHP 贡献**: +2.1% | **状态**: ok | **槽位**: Jewel 61834

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| EnergyShield | INC | 17 | -0.0% | +2.1% |
| ElementalDamage | INC | 15 | +3.2% | -0.0% |
| CurseActivation | INC | 14 | -0.0% | -0.0% |

### Chimeric Spark (Sapphire, RARE)

- **DPS 贡献**: +3.0% | **EHP 贡献**: +1.9% | **状态**: ok | **槽位**: Jewel 7960

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| EnergyShield | INC | 15 | -0.0% | +1.9% |
| ElementalDamage | INC | 14 | +3.0% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +48.3% | +56.2% (辅助+7.9%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +60.0% | +70.0% | +10.0% | 108%→130% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Uhtred's Omen**: +3 level (条件: 1个其他辅助)
- 总辅助贡献: **+7.9%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+10.0%
...
| 3 | Charge Infusion | +14.0% | +14.0% ⚠️模拟 | +0.0% | 30 |
...
- **Elemental Conflux** (Lv21): 分别注入 70% MORE 到火/冰/电取平均。伤害构成：火 30.4% / 冰 56.6% / 电 13.0%
  三次模拟 DPS：火 12701 / 冰 14627 / 电 11429
...
- **构筑已有 modifier**：施法速度 INC 132%（来自 POB skillModList），总 MORE ×0.91。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| 2 | Archmage（大法师） | 100 | +28.9% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Mysticism II | 30 | +8.4% | 需精魄 30（缺 30） ⚠️估算 | 动态扫描 |
| 2 | Mysticism I | 15 | +6.3% | 需精魄 15（缺 15） ⚠️估算 | 动态扫描 |
...
当前 **Frost Wall** TotalDPS = **45,547**，AverageHit = 189,781，Speed = 0.24/s，CritChance = 48.6%，CritMultiplier = 4.00x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +21.6% |
| 2 | crit_multi_base | BASE | 100 | +34% | CritBase 100→134, 需要 +34 → DPS +20.2% |
| 3 | spell_damage_inc | INC | 378 | +95% | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 4 | elemental_damage_inc | INC | 378 | +95% | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 5 | crit_multi_inc | INC | 200 | +102% | INC 200%→302%, 需要 +102 → DPS +20.2% |
...
1. **Stormbreaker**: DPS +16.9%
2. **Harness the Elements**: DPS +16.9%
3. **Endless Blizzard**: DPS +13.9%
4. **Temporal Mastery**: DPS +13.3%
5. **Climate Change**: DPS +12.1%

**⚠️ 17 个无效天赋**: Invocated Echoes, ...and I Shall Rage, Impending Doom, Blood Transfusion, Energise, Heavy Frost, Marked Agility, Shimmering 等 17 个
