...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 4.17% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 4.17% DPS，需要 +4% 达到 +20% DPS
2. **crit_multi_base** (BASE): 每 1% 提升 0.51% DPS，需要 +38% 达到 +20% DPS
3. **crit_chance_inc** (INC): 每 1% 提升 0.31% DPS，需要 +64% 达到 +20% DPS
...
### CritChance BASE = 29.2% (base 11.0% + added 0%)
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 4.17%/% | 0 | baseCrit 0.0%→4.5%, 需要 +4.5% → DPS +18.8% |
| 2 | crit_multi_base | BASE | +38.5% | % | 0.51%/% | 100 | CritBase 100→138, 需要 +38 → DPS +19.7% |
| 3 | crit_chance_inc | INC | +64.5% | % | 0.31%/% | 66 | INC 66%→130%, 需要 +64 → DPS +20.1% |
...
| 7 | crit_multi_inc | INC | +142.0% | % | 0.14%/% | 265 | INC 265%→407%, 需要 +142 → DPS +20.1% |
| 8 | lightning_damage_inc | INC | +493.0% | % | 0.04%/% | 303 | INC 303%→796%, 需要 +493 → DPS +20.0% |
...
| 7 | True Strike | Notable | -6.2% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 8 | Sudden Escalation | Notable | -5.0% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
...
| 10 | For the Jugular | Notable | -3.5% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 11 | Throatseeker | Notable | -3.3% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
...
| 3 | Climate Change | Notable | +18.5% | +0.0% | 输出 |
| 4 | Calculated Hunter | Notable | +15.5% | +0.0% | 输出 |
| 5 | Stormcharged | Notable | +14.8% | +0.0% | 输出 |
| 6 | Endless Blizzard | Notable | +14.2% | +0.0% | 输出 |
| 7 | Temporal Mastery | Notable | +13.3% | +0.0% | 输出 |
| 8 | Biting Frost | Notable | +13.0% | +0.0% | 输出 |
| 9 | Power of the Storm | Notable | +12.2% | +0.0% | 输出 |
| 10 | Limitless Pursuit | Notable | +11.7% | +0.0% | 输出 |
...
| 3 | Charge Infusion | +11.9% | +11.9% ⚠️模拟 | +0.0% | 30 |
...
- **Elemental Conflux** (Lv21): 分别注入 70% MORE 到火/冰/电取平均。伤害构成：火 0.0% / 冰 83.7% / 电 16.4%
  三次模拟 DPS：火 2170 / 冰 3441 / 电 2418
...
| 2 | Archmage（大法师） | 100 | +41.5% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
...
当前 **Frost Wall** TotalDPS = **12,526**，AverageHit = 52,190，Speed = 0.24/s，CritChance = 29.2%，CritMultiplier = 4.65x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→4.5%, 需要 +4.5% → DPS +18.8% |
| 2 | crit_multi_base | BASE | 100 | +38% | CritBase 100→138, 需要 +38 → DPS +19.7% |
| 3 | crit_chance_inc | INC | 66 | +64% | INC 66%→130%, 需要 +64 → DPS +20.1% |
...
3. **Climate Change**: DPS +18.5%
4. **Calculated Hunter**: DPS +15.5%
5. **Stormcharged**: DPS +14.8%
