...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 7.04% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 7.04% DPS，需要 +4% 达到 +20% DPS
2. **crit_multi_base** (BASE): 每 1% 提升 0.44% DPS，需要 +47% 达到 +20% DPS
...
活跃伤害类型: Lightning, Cold, Chaos

### 通用伤害 INC (Lightning,Cold,Chaos) = 226%

**类别汇总**: Tree: +156.0 | Support: +40.0 | Triggered: +30.0
...
### 通用伤害 MORE (Lightning,Cold,Chaos) = +30.0%
...
### CritChance BASE = 29.7% (base 7.0% + added 0%)
...
### Lucky Hits (Lightning,Cold,Chaos) = 20%
...
### Chaos → Lightning Conversion/Gain = Chaos → Lightning: 增益 22.0%
...
### Chaos → Cold Conversion/Gain = Chaos → Cold: 增益 10.0%
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占59.9%) | Enemy | +0.0 |
| 冰霜 +0.0% (占27.6%) | Enemy | +0.0 |
| 混沌 +0.0% (占12.5%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 7.04%/% | 0 | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +24.6% |
| 2 | crit_multi_base | BASE | +47.0% | % | 0.44%/% | 100 | CritBase 100→147, 需要 +47 → DPS +20.5% |
| 3 | cast_speed_inc | INC | +51.5% | % | 0.39%/% | 159 | INC 159%→210%, 需要 +52 → DPS +20.1% |
| 4 | crit_chance_inc | INC | +77.0% | % | 0.26%/% | 66 | INC 66%→143%, 需要 +77 → DPS +20.0% |
| 5 | spell_damage_inc | INC | +80.5% | % | 0.25%/% | 327 | INC 327%→408%, 需要 +80 → DPS +20.2% |
| 6 | elemental_damage_inc | INC | +110.0% | % | 0.18%/% | 327 | INC 327%→437%, 需要 +110 → DPS +20.0% |
| 7 | lightning_damage_inc | INC | +162.0% | % | 0.13%/% | 327 | INC 327%→489%, 需要 +162 → DPS +20.3% |
| 8 | crit_multi_inc | INC | +168.5% | % | 0.12%/% | 265 | INC 265%→434%, 需要 +168 → DPS +20.0% |
| 9 | chaos_damage_inc | INC | +293.5% | % | 0.07%/% | 226 | INC 226%→520%, 需要 +294 → DPS +20.0% |
| 10 | cold_damage_inc | INC | +352.5% | % | 0.06%/% | 337 | INC 337%→690%, 需要 +352 → DPS +20.1% |

**无影响维度**: physical_damage_inc, fire_damage_inc, lightning_pen, fire_pen, cold_pen, elemental_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | I am the Blizzard... | Notable | -24.5% | +0.0% | 输出 | Gain 10% of Damage as Extra Cold Damage; On Freezing Enemies create Chilled Ground |
| 2 | I am the Thunder... | Notable | -24.1% | +0.0% | 输出 | Gain 10% of Damage as Extra Lightning Damage; 25% chance on Shocking Enemies to created Shocked Ground |
| 3 | Thin Ice | Notable | -12.6% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 4 | Crashing Wave | Notable | -8.8% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
| 5 | Sudden Escalation | Notable | -7.1% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 6 | Potent Incantation | Notable | -5.5% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
| 7 | All Natural | Notable | -5.2% | +0.0% | 输出 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 8 | True Strike | Notable | -5.2% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
...
| 12 | For the Jugular | Notable | -2.9% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 13 | Throatseeker | Notable | -2.8% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 14 | The Spring Hare | Notable | -1.7% | +0.0% | 输出 | 20% chance for Damage of Enemies Hitting you to be Unlucky; 20% chance for Damage with Hits to be Lucky |
...
| 2 | Dark Entries | Notable | +21.1% | +0.0% | 输出 |
| 3 | Stormbreaker | Notable | +20.0% | +0.0% | 输出 |
| 4 | Harness the Elements | Notable | +20.0% | +0.0% | 输出 |
| 5 | Volcanic Skin | Notable | +14.6% | +0.0% | 输出 |
| 6 | Molten Being | Notable | +14.6% | +1.6% | 兼顾 |
| 7 | Power of the Storm | Notable | +12.7% | +0.0% | 输出 |
| 8 | The Frenzied Bear | Notable | +11.7% | +0.4% | 兼顾 |
| 9 | Stormcharged | Notable | +11.5% | +0.0% | 输出 |
| 10 | Calculated Hunter | Notable | +10.8% | +0.0% | 输出 |
...
- **DPS 贡献**: +29.1% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 32763

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsLightning | BASE | 12 | +29.1% | -0.0% |
...
- **DPS 贡献**: +1.7% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
- **分配天赋**: the spring hare, savoured blood (DPS +1.7%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | the spring hare | +1.7% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +37.9% | +47.1% (辅助+9.2%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +45.7% | +56.6% | +11.0% | 137%→159% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Uhtred's Omen**: +3 level (条件: 1个其他辅助)
- 总辅助贡献: **+9.2%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+11.0%
...
| 2 | Charge Infusion | +21.5% | +21.5% ⚠️模拟 | +0.0% | 30 |
...
| 3 | Elemental Conflux | +10.8% | +10.8% ⚠️模拟 | +0.0% | 60 |
...
- **Elemental Conflux** (Lv21): 分别注入 70% MORE 到火/冰/电取平均。伤害构成：火 0.0% / 冰 31.4% / 电 68.7%
  三次模拟 DPS：火 699 / 冰 772 / 电 852
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Archmage（大法师） | 100 | +119.7% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
| 2 | Attrition（损耗） | 30 | +59.4% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
...
| 技能 | 效果描述 | DPS 预估 |
|------|----------|----------|
| **Unbound Avatar** | 40% MORE 元素伤害（Unbound 状态） | **+28.6%** |
| **Pinnacle of Power** | 7 × (15% + 20×0.1%) = 119% MORE 元素伤害（PowerChargesMax） | **+85.2%** |
| **Elemental Conflux** | 元素伤害 MORE（期望 59%×1/3≈20%，等级20，简化模拟） | **+14.3%** |
...
当前 **Doedre's Dark Design** TotalDPS = **2,633**，AverageHit = 1,016，Speed = 2.59/s，CritChance = 29.7%，CritMultiplier = 3.55x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +24.6% |
| 2 | crit_multi_base | BASE | 100 | +47% | CritBase 100→147, 需要 +47 → DPS +20.5% |
| 3 | cast_speed_inc | INC | 159 | +52% | INC 159%→210%, 需要 +52 → DPS +20.1% |
| 4 | crit_chance_inc | INC | 66 | +77% | INC 66%→143%, 需要 +77 → DPS +20.0% |
| 5 | spell_damage_inc | INC | 327 | +80% | INC 327%→408%, 需要 +80 → DPS +20.2% |
...
2. **Dark Entries**: DPS +21.1%
3. **Stormbreaker**: DPS +20.0%
4. **Harness the Elements**: DPS +20.0%
5. **Volcanic Skin**: DPS +14.6%
...
**最佳**: Heart of the Well (DPS +29.1%, EHP -0.0%)
