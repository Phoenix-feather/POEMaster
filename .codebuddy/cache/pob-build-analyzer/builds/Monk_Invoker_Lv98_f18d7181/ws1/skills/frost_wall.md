...
1. **crit_chance_base** (BASE): 每 1% 提升 7.17% DPS，需要 +4% 达到 +20% DPS
2. **crit_multi_base** (BASE): 每 1% 提升 0.69% DPS，需要 +30% 达到 +20% DPS
...
### 元素伤害 MORE (Lightning,Cold,Fire) = +741.8%
...
**类别汇总**: Item: +64.0 | Aura: +50.0 | Sim: +25.0 | Tree: +19.0
...
### CritChance BASE = 61.8% (base 11.0% + added 0%)
...
**公式**: `基础 10792-16134 × 0.2层 × effMult 1.6770  (持续 4.40s, 几率 24.0%)`
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +3.5% | % | 7.17%/% | 0 | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +25.1% |
| 2 | crit_multi_base | BASE | +30.0% | % | 0.69%/% | 100 | CritBase 100→130, 需要 +30 → DPS +20.7% |
...
| 5 | crit_chance_inc | INC | +102.0% | % | 0.20%/% | 251 | INC 251%→353%, 需要 +102 → DPS +20.0% |
| 6 | crit_multi_inc | INC | +105.0% | % | 0.19%/% | 260 | INC 260%→365%, 需要 +105 → DPS +20.1% |
...
| 2 | Dynamism | Notable | -8.4% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 3 | Throatseeker | Notable | -8.2% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 4 | Crashing Wave | Notable | -7.6% | +0.0% | 输出 | 36% increased Damage if you've dealt a Critical Hit in the past 8 seconds |
| 5 | Deadly Force | Notable | -7.1% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
| 6 | Careful Assassin | Notable | -6.5% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 7 | Potent Incantation | Notable | -6.3% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
| 8 | All Natural | Notable | -6.3% | -4.2% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 9 | Critical Exploit | Notable | -4.9% | +0.0% | 输出 | 25% increased Critical Hit Chance |
| 10 | Evocational Practitioner | Notable | -4.9% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
| 11 | I am the Blizzard... | Notable | -4.9% | +0.0% | 输出 | Gain 10% of Damage as Extra Cold Damage; On Freezing Enemies create Chilled Ground |
| 12 | I am the Thunder... | Notable | -4.8% | +0.0% | 输出 | Gain 10% of Damage as Extra Lightning Damage; 25% chance on Shocking Enemies to created Shocked Ground |
| 13 | For the Jugular | Notable | -4.8% | -0.4% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
...
| 15 | True Strike | Notable | -3.9% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 16 | Sudden Escalation | Notable | -3.1% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 17 | Moment of Truth | Notable | -2.9% | +0.0% | 输出 | 25% increased Critical Damage Bonus if you've dealt a Non-Critical Hit Recently; 15% increased Critical Hit Chance |
...
| 6 | Limitless Pursuit | Notable | +11.7% | +0.0% | 输出 |
| 7 | Stormcharged | Notable | +11.6% | +0.0% | 输出 |
| 8 | Cooked | Notable | +11.5% | -4.6% | 兼顾 |
...
- **DPS 贡献**: +3.2% | **EHP 贡献**: +2.4% | **状态**: ok | **槽位**: Jewel 61834
...
- **DPS 贡献**: +3.0% | **EHP 贡献**: +2.1% | **状态**: ok | **槽位**: Jewel 7960
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +48.1% | +56.2% (辅助+8.1%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +60.1% | +70.0% | +9.9% | 134%→156% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Uhtred's Omen**: +3 level (条件: 1个其他辅助)
- 总辅助贡献: **+8.1%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+9.9%
...
| 3 | Charge Infusion | +16.6% | +16.6% ⚠️模拟 | +0.0% | 30 |
...
- **Elemental Conflux** (Lv21): 分别注入 70% MORE 到火/冰/电取平均。伤害构成：火 30.4% / 冰 56.6% / 电 13.0%
  三次模拟 DPS：火 11753 / 冰 13534 / 电 10576
...
- **构筑已有 modifier**：施法速度 INC 158%（来自 POB skillModList），总 MORE ×0.91。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
- Charge Infusion 使用构筑实际等级 Lv21（效果值=25），非满级 Lv20
...
当前 **Frost Wall** TotalDPS = **58,292**，AverageHit = 242,885，Speed = 0.24/s，CritChance = 61.8%，CritMultiplier = 4.60x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +25.1% |
| 2 | crit_multi_base | BASE | 100 | +30% | CritBase 100→130, 需要 +30 → DPS +20.7% |
| 3 | spell_damage_inc | INC | 378 | +95% | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 4 | elemental_damage_inc | INC | 378 | +95% | INC 378%→473%, 需要 +95 → DPS +20.1% |
| 5 | crit_chance_inc | INC | 251 | +102% | INC 251%→353%, 需要 +102 → DPS +20.0% |
