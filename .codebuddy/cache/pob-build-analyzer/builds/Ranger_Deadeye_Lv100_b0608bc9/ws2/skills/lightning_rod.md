...
4. 💡 **elemental_pen** 对 DPS 影响最大（每 1% 提升 2.11% DPS）

### 优化方向 Top 3

1. **elemental_pen** (BASE): 每 1% 提升 2.11% DPS，需要 +10% 达到 +20% DPS
2. **lightning_pen** (BASE): 每 1% 提升 1.82% DPS，需要 +10% 达到 +20% DPS
3. **crit_chance_base** (BASE): 每 1% 提升 1.49% DPS，需要 +14% 达到 +20% DPS
...
### Lightning Base Damage = 19-556 (x1.48 base mult)
...
### Cold Base Damage = 55-86 (x1.48 base mult)
...
### Fire Base Damage = 24-47 (x1.48 base mult)
...
### 通用伤害 INC (Lightning,Cold,Fire) = 594%
...
### 元素伤害 MORE (Lightning,Cold,Fire) = +56.0%
...
| Elemental Focus | Support | +25.0% MORE |
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占95.7%) | Enemy | +0.0 |
| 冰霜 +0.0% (占3.2%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | elemental_pen | BASE | +9.5% | % | 2.11%/% | 0 | 需要 +10% 穿透 → DPS +20.0% |
| 2 | lightning_pen | BASE | +10.5% | % | 1.82%/% | 0 | 需要 +10% 穿透 → DPS +19.1% |
| 3 | crit_chance_base | BASE | +13.5% | % | 1.49%/% | 0 | baseCrit 0.0%→13.5%, 需要 +13.5% → DPS +20.1% |
| 4 | attack_speed_inc | INC | +45.5% | % | 0.44%/% | 129 | INC 129%→174%, 需要 +46 → DPS +20.1% |
| 5 | flat_lightning_attack | BASE | +75.5 |  | 0.26%/1 | 0 | 需要添加 76-151 基础伤害 → DPS +20.0% |
| 6 | flat_physical_attack | BASE | +76.0 |  | 0.26%/1 | 0 | 需要添加 76-152 基础伤害 → DPS +20.0% |
| 7 | flat_fire_attack | BASE | +117.0 |  | 0.17%/1 | 0 | 需要添加 117-234 基础伤害 → DPS +20.1% |
| 8 | flat_cold_attack | BASE | +117.0 |  | 0.17%/1 | 0 | 需要添加 117-234 基础伤害 → DPS +20.1% |
| 9 | attack_damage_inc | INC | +170.0% | % | 0.12%/% | 594 | INC 594%→764%, 需要 +170 → DPS +20.0% |
| 10 | projectile_damage_inc | INC | +170.0% | % | 0.12%/% | 594 | INC 594%→764%, 需要 +170 → DPS +20.0% |
| 11 | elemental_damage_inc | INC | +170.0% | % | 0.12%/% | 594 | INC 594%→764%, 需要 +170 → DPS +20.0% |
| 12 | lightning_damage_inc | INC | +179.0% | % | 0.11%/% | 594 | INC 594%→773%, 需要 +179 → DPS +20.0% |
| 13 | crit_multi_inc | INC | +279.0% | % | 0.07%/% | 0 | INC 0%→279%, 需要 +279 → DPS +20.1% |
| 14 | crit_chance_inc | INC | +431.0% | % | 0.05%/% | 55 | INC 55%→486%, 需要 +431 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Feathered Fletching | Notable | -16.6% | +0.0% | 输出 | Increases and Reductions to Projectile Speed also apply to Damage with Bows |
| 2 | Wrapped Quiver | Notable | -11.3% | +0.0% | 输出 | 20% increased bonuses gained from Equipped Quiver |
| 3 | Master Fletching | Notable | -11.3% | +0.0% | 输出 | 20% increased bonuses gained from Equipped Quiver |
| 4 | Inspiring Ally | Notable | -11.0% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 5 | Killer Instinct | Notable | -4.7% | +0.0% | 输出 | 40% increased Attack Damage while on Full Life; 60% increased Attack Damage while on Low Life |
| 6 | Coming Calamity | Notable | -4.5% | +0.0% | 输出 | 40% increased Cold Damage while affected by Herald of Ice; 40% increased Fire Damage while affected by Herald of Ash; 40% increased Lightning Damage while affected by Herald of Thunder |
| 7 | Acceleration | Notable | -4.4% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 8 | Wild Storm | Notable | -3.6% | +0.0% | 输出 | Gain 4% of Damage as Extra Cold Damage; Gain 4% of Damage as Extra Lightning Damage; +10 to Dexterity |
| 9 | Kite Runner | Notable | -3.5% | +0.0% | 输出 | 3% increased Movement Speed; 15% increased Projectile Speed; 15% increased Projectile Damage |
...
| 12 | Maiming Strike | Notable | -2.9% | +0.0% | 输出 | 25% increased Attack Damage; Attacks have 25% chance to Maim on Hit |
| 13 | Catalysis | Notable | -2.4% | -1.9% | 兼顾 | 20% increased Elemental Damage with Attacks; 5% of Physical Damage from Hits taken as Damage of a Random Element |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Electric Amplification | Notable | +39.6% | +0.0% | 输出 |
| 2 | Exposed to the Storm | Notable | +34.5% | +0.0% | 输出 |
| 3 | Dimensional Weakspot | Notable | +30.0% | +0.0% | 输出 |
| 4 | Forces of Nature | Notable | +30.0% | +0.0% | 输出 |
| 5 | Storm Surge | Notable | +29.2% | +0.0% | 输出 |
| 6 | Breath of Lightning | Notable | +28.7% | +0.0% | 输出 |
| 7 | Flash Storm | Notable | +28.7% | +0.0% | 输出 |
| 8 | Surging Currents | Notable | +28.7% | +0.0% | 输出 |
| 9 | Overload | Notable | +28.7% | +0.0% | 输出 |
| 10 | Primal Sundering | Notable | +24.0% | +0.0% | 输出 |
...
- **DPS 贡献**: +9.4% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 60735

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsLightning | BASE | 15 | +9.4% | -0.0% |
| LifeOnKill | BASE | 1 | -0.0% | -0.0% |
| ManaOnKill | BASE | 1 | -0.0% | -0.0% |

### Empyrean Star (Emerald, RARE)

- **DPS 贡献**: +5.0% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 46882

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| Speed | INC | 3 | +1.3% | -0.0% |
| MovementSpeed | INC | 2 | -0.0% | -0.0% |
| Damage | INC | 13 | +1.5% | -0.0% |
...
| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| MinionModifier | LIST | (complex data) | +1.6% | -0.0% |
| MovementSpeed | INC | 2 | -0.0% | -0.0% |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Trinity（三位一体） | 100 | +100.0% | +0.0% | 需精魄 100（缺 100）; 三属性穿透（需要 ResonanceCount 配置） |
| 2 | Attrition（损耗） | 30 | +59.0% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
| 3 | Elemental Conflux（元素交融） | 60 | +20.0% | +0.0% | 需精魄 60（缺 60）; 元素异常状态同步 |
| 4 | Berserk（狂暴） | 30 | +17.0% | +0.0% | 需精魄 30（缺 30）; MORE Damage + 受伤增加 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +8.2% | 需精魄 40（缺 40）; Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +5.9% | 需精魄 20（缺 20）; Low Life | 硬编码 |
...
当前 **Lightning Rod** TotalDPS = **44,646**，AverageHit = 0，Speed = 3.52/s，CritChance = 7.8%，CritMultiplier = 2.00x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | elemental_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +20.0% |
| 2 | lightning_pen | BASE | 0 | +10% | 需要 +10% 穿透 → DPS +19.1% |
| 3 | crit_chance_base | BASE | 0 | +14% | baseCrit 0.0%→13.5%, 需要 +13.5% → DPS +20.1% |
| 4 | attack_speed_inc | INC | 129 | +46% | INC 129%→174%, 需要 +46 → DPS +20.1% |
| 5 | flat_lightning_attack | BASE | 0 | +76 | 需要添加 76-151 基础伤害 → DPS +20.0% |
...
1. **Electric Amplification**: DPS +39.6%
2. **Exposed to the Storm**: DPS +34.5%
3. **Dimensional Weakspot**: DPS +30.0%
4. **Forces of Nature**: DPS +30.0%
5. **Storm Surge**: DPS +29.2%
...
**最佳**: Heart of the Well (DPS +9.4%, EHP -0.0%)
**可替换**: From Nothing
