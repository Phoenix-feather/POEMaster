...
2. **elemental_pen** (BASE): 每 1% 提升 1.08% DPS，需要 +20% 达到 +20% DPS
3. **lightning_pen** (BASE): 每 1% 提升 0.87% DPS，需要 +22% 达到 +20% DPS
...
### Physical Base Damage = 500-851 (x2.09 base mult)
...
### Lightning Base Damage = 27-786 (x2.09 base mult)
...
### Cold Base Damage = 77-121 (x2.09 base mult)
...
### Fire Base Damage = 33-67 (x2.09 base mult)
...
| 来源 | 类别 | 值 |
|------|------|-----|
| Concentrated Area | Support | +30.0% MORE |
...
**类别汇总**: SkillEffect: +3.0

| 来源 | 类别 | 值 |
|------|------|-----|
| Tornado效果 | SkillEffect | +3.0 |
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +0.0% (占67.3%) | Enemy | +0.0 |
| 物理 +0.0% (占20.2%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +13.5% | % | 1.57%/% | 0 | baseCrit 0.0%→13.5%, 需要 +13.5% → DPS +21.2% |
| 2 | elemental_pen | BASE | +19.5% | % | 1.08%/% | 0 | 需要 +20% 穿透 → DPS +21.0% |
| 3 | lightning_pen | BASE | +22.5% | % | 0.87%/% | 0 | 需要 +22% 穿透 → DPS +19.5% |
...
| 6 | flat_lightning_attack | BASE | +91.5 |  | 0.22%/1 | 0 | 需要添加 92-183 基础伤害 → DPS +20.1% |
| 7 | flat_fire_attack | BASE | +103.0 |  | 0.20%/1 | 0 | 需要添加 103-206 基础伤害 → DPS +20.1% |
| 8 | flat_cold_attack | BASE | +103.0 |  | 0.19%/1 | 0 | 需要添加 103-206 基础伤害 → DPS +20.1% |
| 9 | attack_damage_inc | INC | +144.5% | % | 0.14%/% | 594 | INC 594%→738%, 需要 +144 → DPS +20.1% |
| 10 | projectile_damage_inc | INC | +144.5% | % | 0.14%/% | 594 | INC 594%→738%, 需要 +144 → DPS +20.1% |
| 11 | physical_damage_inc | INC | +263.0% | % | 0.08%/% | 594 | INC 594%→857%, 需要 +263 → DPS +20.0% |
| 12 | crit_multi_inc | INC | +263.5% | % | 0.08%/% | 0 | INC 0%→264%, 需要 +264 → DPS +20.1% |
| 13 | elemental_damage_inc | INC | +317.5% | % | 0.06%/% | 594 | INC 594%→912%, 需要 +318 → DPS +20.0% |
| 14 | lightning_damage_inc | INC | +387.0% | % | 0.05%/% | 594 | INC 594%→981%, 需要 +387 → DPS +20.0% |
| 15 | crit_chance_inc | INC | +409.5% | % | 0.05%/% | 55 | INC 55%→464%, 需要 +410 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Feathered Fletching | Notable | -19.5% | +0.0% | 输出 | Increases and Reductions to Projectile Speed also apply to Damage with Bows |
| 2 | Inspiring Ally | Notable | -12.9% | +0.0% | 输出 | Increases and Reductions to Companion Damage also apply to you |
| 3 | Wrapped Quiver | Notable | -10.7% | +0.0% | 输出 | 20% increased bonuses gained from Equipped Quiver |
| 4 | Master Fletching | Notable | -10.7% | +0.0% | 输出 | 20% increased bonuses gained from Equipped Quiver |
| 5 | Killer Instinct | Notable | -5.5% | +0.0% | 输出 | 40% increased Attack Damage while on Full Life; 60% increased Attack Damage while on Low Life |
| 6 | Wild Storm | Notable | -4.5% | +0.0% | 输出 | Gain 4% of Damage as Extra Cold Damage; Gain 4% of Damage as Extra Lightning Damage; +10 to Dexterity |
| 7 | Acceleration | Notable | -4.4% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 8 | Kite Runner | Notable | -4.2% | +0.0% | 输出 | 3% increased Movement Speed; 15% increased Projectile Speed; 15% increased Projectile Damage |
| 9 | Flow Like Water | Notable | -3.5% | +0.0% | 输出 | 8% increased Attack and Cast Speed; +5 to Dexterity and Intelligence |
| 10 | Maiming Strike | Notable | -3.5% | +0.0% | 输出 | 25% increased Attack Damage; Attacks have 25% chance to Maim on Hit |
...
| 12 | Flow State | Notable | -2.2% | +0.0% | 输出 | 5% increased Skill Speed; 15% increased Mana Regeneration Rate |
| 13 | Coming Calamity | Notable | -2.1% | +0.0% | 输出 | 40% increased Cold Damage while affected by Herald of Ice; 40% increased Fire Damage while affected by Herald of Ash; 40% increased Lightning Damage while affected by Herald of Thunder |
| 14 | Catalysis | Notable | -1.3% | -1.9% | 兼顾 | 20% increased Elemental Damage with Attacks; 5% of Physical Damage from Hits taken as Damage of a Random Element |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Relinquish Your Life | Notable | +20.7% | +0.0% | 输出 |
| 2 | Electric Amplification | Notable | +18.2% | +0.0% | 输出 |
| 3 | Exposed to the Storm | Notable | +15.9% | +0.0% | 输出 |
| 4 | Dimensional Weakspot | Notable | +15.7% | +0.0% | 输出 |
| 5 | Forces of Nature | Notable | +15.7% | +0.0% | 输出 |
| 6 | Storm Surge | Notable | +14.3% | +0.0% | 输出 |
| 7 | Breath of Lightning | Notable | +13.3% | +0.0% | 输出 |
| 8 | Flash Storm | Notable | +13.3% | +0.0% | 输出 |
| 9 | Surging Currents | Notable | +13.3% | +0.0% | 输出 |
| 10 | Overload | Notable | +13.3% | +0.0% | 输出 |
...
- **DPS 贡献**: +9.1% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 60735

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| DamageGainAsLightning | BASE | 15 | +9.1% | -0.0% |
| LifeOnKill | BASE | 1 | -0.0% | -0.0% |
| ManaOnKill | BASE | 1 | -0.0% | -0.0% |

### Empyrean Star (Emerald, RARE)

- **DPS 贡献**: +5.7% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 46882

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| Speed | INC | 3 | +1.3% | -0.0% |
| MovementSpeed | INC | 2 | -0.0% | -0.0% |
| Damage | INC | 13 | +1.8% | -0.0% |
| MinionModifier | LIST | (complex data) | +2.6% | -0.0% |

### Hypnotic Shine (Emerald, RARE)

- **DPS 贡献**: +4.9% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 32763

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| MinionModifier | LIST | (complex data) | +1.9% | -0.0% |
| MovementSpeed | INC | 2 | -0.0% | -0.0% |
| Damage | INC | 15 | +2.1% | -0.0% |
| Speed | INC | 2 | +0.9% | -0.0% |
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +62.1% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
| 2 | Trinity（三位一体） | 100 | +52.5% | +0.0% | 需精魄 100（缺 100）; 三属性穿透（需要 ResonanceCount 配置） |
| 3 | Berserk（狂暴） | 30 | +17.9% | +0.0% | 需精魄 30（缺 30）; MORE Damage + 受伤增加 |
| 4 | Charge Infusion（充能灌注） | 30 | +15.9% | +32.4% | 需精魄 30（缺 30）; Frenzy/Power/Endurance Charge 增益 |
| 5 | Elemental Conflux（元素交融） | 60 | +10.5% | +0.0% | 需精魄 60（缺 60）; 元素异常状态同步 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +9.7% | 需精魄 40（缺 40）; Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +6.9% | 需精魄 20（缺 20）; Low Life | 硬编码 |
...
当前 **Tornado Shot** TotalDPS = **54,547**，AverageHit = 0，Speed = 3.52/s，CritChance = 7.8%，CritMultiplier = 2.00x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +14% | baseCrit 0.0%→13.5%, 需要 +13.5% → DPS +21.2% |
| 2 | elemental_pen | BASE | 0 | +20% | 需要 +20% 穿透 → DPS +21.0% |
| 3 | lightning_pen | BASE | 0 | +22% | 需要 +22% 穿透 → DPS +19.5% |
...
1. **Relinquish Your Life**: DPS +20.7%
2. **Electric Amplification**: DPS +18.2%
3. **Exposed to the Storm**: DPS +15.9%
4. **Dimensional Weakspot**: DPS +15.7%
5. **Forces of Nature**: DPS +15.7%
...
**最佳**: Heart of the Well (DPS +9.1%, EHP -0.0%)
**可替换**: From Nothing
