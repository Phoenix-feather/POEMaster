...
5. 💡 **crit_chance_base** 对 DPS 影响最大（每 1% 提升 4.05% DPS）

### 优化方向 Top 3

1. **crit_chance_base** (BASE): 每 1% 提升 4.05% DPS，需要 +4% 达到 +20% DPS
2. **elemental_pen** (BASE): 每 1% 提升 1.32% DPS，需要 +14% 达到 +20% DPS
3. **fire_pen** (BASE): 每 1% 提升 0.84% DPS，需要 +24% 达到 +20% DPS
...
### 通用伤害 INC (Physical,Lightning,Cold,Fire) = 462%

**类别汇总**: Tree: +280.0 | Jewel: +80.0 | Item: +62.0 | Skill: +40.0
...
### Physical → Lightning Conversion/Gain = Physical → Lightning: 增益 5.0%
...
### Physical → Cold Conversion/Gain = Physical → Cold: 增益 35.0%
...
### Physical → Fire Conversion/Gain = Physical → Fire: 增益 65.0%
...
**公式**: `基础 4145-7691 × 效果 1.1100 × effMult 1.1200  (持续 3.56s, 几率 12.2%)`

**类别汇总**: Sim: +2900.0 | Tree: +185.0 | Jewel: +80.0 | Item: +11.0
...
### Combined DPS = 158,873

**类别汇总**: Hit: +152958.6 | DOT: +5914.0
...
**类别汇总**: Penetration: +13.0

| 来源 | 类别 | 值 |
|------|------|-----|
| 火焰 +16.0% (占55.1%) | Enemy | +16.0 |
| 冰霜 +16.0% (占30.1%) | Enemy | +16.0 |
| 物理 +0.0% (占10.5%) | Enemy | +0.0 |
| 闪电 +16.0% (占4.4%) | Enemy | +16.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | crit_chance_base | BASE | +4.5% | % | 4.05%/% | 0 | baseCrit 0.0%→4.5%, 需要 +4.5% → DPS +18.2% |
| 2 | elemental_pen | BASE | +14.5% | % | 1.32%/% | 8 | 需要 +14% 穿透 → DPS +19.2% |
| 3 | fire_pen | BASE | +24.0% | % | 0.84%/% | 0 | 需要 +24% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | +28.5% | % | 0.68%/% | 100 | CritBase 100→128, 需要 +28 → DPS +19.3% |
| 5 | cast_speed_inc | INC | +45.5% | % | 0.45%/% | 126 | INC 126%→172%, 需要 +46 → DPS +20.4% |
| 6 | crit_multi_inc | INC | +87.5% | % | 0.23%/% | 202 | INC 202%→290%, 需要 +88 → DPS +20.0% |
| 7 | crit_chance_inc | INC | +102.5% | % | 0.20%/% | 249 | INC 249%→352%, 需要 +102 → DPS +20.1% |
| 8 | spell_damage_inc | INC | +133.0% | % | 0.15%/% | 623 | INC 623%→756%, 需要 +133 → DPS +20.0% |
| 9 | elemental_damage_inc | INC | +183.5% | % | 0.11%/% | 623 | INC 623%→806%, 需要 +184 → DPS +20.0% |
| 10 | fire_damage_inc | INC | +296.5% | % | 0.07%/% | 623 | INC 623%→920%, 需要 +296 → DPS +20.0% |
| 11 | physical_damage_inc | INC | +480.0% | % | 0.04%/% | 462 | INC 462%→942%, 需要 +480 → DPS +20.0% |

**无影响维度**: cold_damage_inc, lightning_damage_inc, chaos_damage_inc, lightning_pen, cold_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Stormbreaker | Notable | -12.0% | +0.0% | 输出 | 20% increased Damage for each type of Elemental Ailment on Enemy |
| 2 | Throatseeker | Notable | -10.5% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 3 | Gore Spike | Notable | -8.0% | +0.0% | 输出 | 1% increased Critical Damage Bonus per 50 current Life |
| 4 | Thin Ice | Notable | -7.5% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 5 | Stormcharged | Notable | -7.2% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
| 6 | Sudden Escalation | Notable | -6.6% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 7 | Dynamism | Notable | -6.0% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 8 | Careful Assassin | Notable | -5.9% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 9 | For the Jugular | Notable | -5.7% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 10 | Deadly Force | Notable | -5.7% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
| 11 | Critical Overload | Notable | -5.1% | +0.0% | 输出 | 15% increased Critical Hit Chance for Spells; 15% increased Spell Damage if you've dealt a Critical Hit Recently |
| 12 | Evocational Practitioner | Notable | -4.9% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
| 13 | True Strike | Notable | -3.9% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 14 | All Natural | Notable | -3.3% | +0.0% | 输出 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| 16 | Practiced Signs | Notable | -2.7% | +0.0% | 输出 | 6% increased Cast Speed |
| 17 | Potent Incantation | Notable | -2.4% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
...
Invocated Echoes, Sacrificial Blood, The Spring Hare, Blood Transfusion, Echoing Thunder, Energise, Heavy Frost, Echoing Frost, Invocated Efficiency, Overflowing Power, Sunder the Flesh, Echoing Flames, Sanguine Tides, Vitality Siphon, Marked Agility, Acceleration, Infusion of Power, Sanguimancy

## 7. 未分配天赋探索

| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Dimensional Weakspot | Notable | +20.5% | +0.0% | 输出 |
| 2 | Primal Sundering | Notable | +16.4% | +0.0% | 输出 |
| 3 | Exposed to the Inferno | Notable | +15.2% | +0.0% | 输出 |
| 4 | Cremation | Notable | +15.2% | +0.0% | 输出 |
| 5 | Unquenchable Iron | Notable | +13.9% | +0.0% | 输出 |
| 6 | Cooked | Notable | +13.6% | -5.4% | 兼顾 |
| 7 | Breath of Fire | Notable | +12.9% | +0.3% | 兼顾 |
| 8 | Smoke Inhalation | Notable | +12.6% | +0.0% | 输出 |
| 9 | Chakra of Elements | Notable | +12.4% | +0.0% | 输出 |
| 10 | Deep Freeze | Notable | +10.9% | +0.0% | 输出 |
...
- **DPS 贡献**: +12.0% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
- **分配天赋**: invocated limit, harness the elements (DPS +12.0%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | invocated limit | -0.0% | -0.0% |
| GrantedPassive | LIST | harness the elements | +12.0% | -0.0% |

### Blight Wound (Sapphire, RARE)

- **DPS 贡献**: +3.6% | **EHP 贡献**: +3.5% | **状态**: ok | **槽位**: Jewel 7960
...
- **DPS 贡献**: +3.7% | **EHP 贡献**: +3.0% | **状态**: ok | **槽位**: Jewel 61834

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| Damage | INC | 12 | +1.8% | -0.0% |
| EnergyShield | INC | 17 | -0.0% | +3.0% |
| CritChance | INC | 10 | +2.0% | -0.0% |

### Heart of the Well (Diamond, UNIQUE)

- **DPS 贡献**: +2.7% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 32763

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| CritMultiplier | INC | 12 | +2.7% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +54.6% | +59.5% (辅助+4.9%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +66.4% | +71.7% | +5.3% | 104%→126% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Lightning Mastery**: +1 level
- 总辅助贡献: **+4.9%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+5.3%
...
| 2 | Charge Infusion | +31.7% | +31.7% ⚠️模拟 | +0.0% | 30 |
...
| 3 | Elemental Conflux | +16.8% | +16.8% ⚠️模拟 | +0.0% | 60 |
...
- **Elemental Conflux** (Lv25): 分别注入 74% MORE 到火/冰/电取平均。伤害构成：火 61.5% / 冰 33.6% / 电 4.9%
  三次模拟 DPS：火 86671 / 冰 77380 / 电 67832
...
- **构筑已有 modifier**：施法速度 INC 126%（来自 POB skillModList），总 MORE ×1.30。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +60.9% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
| 2 | Archmage（大法师） | 100 | +38.9% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Mysticism II | 30 | +6.0% | 需精魄 30（缺 30） ⚠️估算 | 动态扫描 |
| 2 | Mysticism I | 15 | +4.5% | 需精魄 15（缺 15） ⚠️估算 | 动态扫描 |
...
| 技能 | 效果描述 | DPS 预估 |
|------|----------|----------|
| **Pinnacle of Power** | 7 × (15% + 20×0.1%) = 119% MORE 元素伤害（PowerChargesMax） | **+101.3%** |
...
当前 **Power Siphon** TotalDPS = **152,959**，AverageHit = 47,377，Speed = 3.23/s，CritChance = 66.5%，CritMultiplier = 4.02x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→4.5%, 需要 +4.5% → DPS +18.2% |
| 2 | elemental_pen | BASE | 8 | +14% | 需要 +14% 穿透 → DPS +19.2% |
| 3 | fire_pen | BASE | 0 | +24% | 需要 +24% 穿透 → DPS +20.2% |
| 4 | crit_multi_base | BASE | 100 | +28% | CritBase 100→128, 需要 +28 → DPS +19.3% |
| 5 | cast_speed_inc | INC | 126 | +46% | INC 126%→172%, 需要 +46 → DPS +20.4% |
...
1. **Dimensional Weakspot**: DPS +20.5%
2. **Primal Sundering**: DPS +16.4%
3. **Exposed to the Inferno**: DPS +15.2%
4. **Cremation**: DPS +15.2%
5. **Unquenchable Iron**: DPS +13.9%

**⚠️ 18 个无效天赋**: Invocated Echoes, Sacrificial Blood, The Spring Hare, Blood Transfusion, Echoing Thunder, Energise, Heavy Frost, Echoing Frost 等 18 个

### 💎 珠宝

**最佳**: Megalomaniac (DPS +12.0%, EHP -0.0%)
