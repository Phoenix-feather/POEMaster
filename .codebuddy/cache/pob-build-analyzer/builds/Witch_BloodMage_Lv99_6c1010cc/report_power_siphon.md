...
1. **elemental_pen** (BASE): 每 1% 提升 1.30% DPS，需要 +14% 达到 +20% DPS
2. **fire_pen** (BASE): 每 1% 提升 0.81% DPS，需要 +24% 达到 +20% DPS
3. **crit_multi_base** (BASE): 每 1% 提升 0.77% DPS，需要 +26% 达到 +20% DPS
...
**公式**: `基础 4008-7437 × 效果 1.1100 × effMult 1.1200  (持续 3.56s, 几率 16.1%)`
...
### Combined DPS = 235,493

**类别汇总**: Hit: +228258.3 | DOT: +7234.3
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 火焰 +16.0% (占54.9%) | Enemy | +16.0 |
| 冰霜 +16.0% (占30.0%) | Enemy | +16.0 |
| 物理 +0.0% (占10.8%) | Enemy | +0.0 |
...
|---|------|------|--------|------|----------|--------|------|
| 1 | elemental_pen | BASE | +14.5% | % | 1.30%/% | 8 | 需要 +14% 穿透 → DPS +18.9% |
| 2 | fire_pen | BASE | +24.5% | % | 0.81%/% | 0 | 需要 +24% 穿透 → DPS +19.9% |
| 3 | crit_multi_base | BASE | +26.0% | % | 0.77%/% | 100 | CritBase 100→126, 需要 +26 → DPS +20.1% |
| 4 | cast_speed_inc | INC | +50.5% | % | 0.40%/% | 153 | INC 153%→204%, 需要 +50 → DPS +20.2% |
| 5 | crit_multi_inc | INC | +93.5% | % | 0.21%/% | 262 | INC 262%→356%, 需要 +94 → DPS +20.1% |
| 6 | spell_damage_inc | INC | +133.5% | % | 0.15%/% | 623 | INC 623%→756%, 需要 +134 → DPS +20.1% |
| 7 | elemental_damage_inc | INC | +186.0% | % | 0.11%/% | 623 | INC 623%→809%, 需要 +186 → DPS +20.0% |
| 8 | fire_damage_inc | INC | +300.5% | % | 0.07%/% | 623 | INC 623%→924%, 需要 +300 → DPS +20.0% |
| 9 | physical_damage_inc | INC | +468.0% | % | 0.04%/% | 462 | INC 462%→930%, 需要 +468 → DPS +20.0% |

**无影响维度**: cold_damage_inc, lightning_damage_inc, chaos_damage_inc, crit_chance_inc, crit_chance_base, lightning_pen, cold_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
| 3 | Thin Ice | Notable | -7.5% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 4 | Stormcharged | Notable | -7.5% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
| 5 | Gore Spike | Notable | -7.5% | +0.0% | 输出 | 1% increased Critical Damage Bonus per 50 current Life |
| 6 | Careful Assassin | Notable | -7.4% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 7 | Sudden Escalation | Notable | -6.6% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 8 | Dynamism | Notable | -6.0% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 9 | Deadly Force | Notable | -5.9% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
...
| 11 | Critical Overload | Notable | -5.5% | +0.0% | 输出 | 15% increased Critical Hit Chance for Spells; 15% increased Spell Damage if you've dealt a Critical Hit Recently |
| 12 | For the Jugular | Notable | -5.3% | -0.2% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 13 | True Strike | Notable | -4.4% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 14 | All Natural | Notable | -3.2% | -5.3% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| 16 | Potent Incantation | Notable | -2.6% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
| 17 | Practiced Signs | Notable | -2.4% | +0.0% | 输出 | 6% increased Cast Speed |
...
Invocated Echoes, Sacrificial Blood, The Spring Hare, Blood Transfusion, Echoing Thunder, Energise, Heavy Frost, Echoing Frost, Invocated Efficiency, Sunder the Flesh, Echoing Flames, Sanguine Tides, Vitality Siphon, Marked Agility, Acceleration, Infusion of Power, Sanguimancy

## 7. 未分配天赋探索

| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Dimensional Weakspot | Notable | +20.2% | +0.0% | 输出 |
| 2 | Primal Sundering | Notable | +16.2% | +0.0% | 输出 |
| 3 | Exposed to the Inferno | Notable | +14.9% | +0.0% | 输出 |
| 4 | Cremation | Notable | +14.9% | +0.0% | 输出 |
| 5 | Unquenchable Iron | Notable | +13.7% | +0.0% | 输出 |
| 6 | Cooked | Notable | +12.8% | -6.7% | 兼顾 |
| 7 | Breath of Fire | Notable | +12.7% | +0.3% | 兼顾 |
| 8 | Smoke Inhalation | Notable | +12.4% | +0.0% | 输出 |
| 9 | Chakra of Elements | Notable | +12.3% | +0.0% | 输出 |
| 10 | Deep Freeze | Notable | +10.8% | +0.0% | 输出 |
...
- **DPS 贡献**: +4.0% | **EHP 贡献**: +3.7% | **状态**: ok | **槽位**: Jewel 7960

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| EnergyShield | INC | 20 | -0.0% | +3.7% |
| CritChance | INC | 14 | +3.1% | -0.0% |
| AilmentMagnitude | INC | 11 | +0.9% | -0.0% |

### Bramble Cut (Sapphire, RARE)

- **DPS 贡献**: +4.0% | **EHP 贡献**: +3.2% | **状态**: ok | **槽位**: Jewel 61834
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +48.1% | +52.3% (辅助+4.2%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +58.5% | +63.0% | +4.5% | 131%→153% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Lightning Mastery**: +1 level
- 总辅助贡献: **+4.2%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+4.5%
...
| 2 | Charge Infusion | +34.1% | +34.1% ⚠️模拟 | +0.0% | 30 |
...
| 3 | Elemental Conflux | +14.6% | +14.6% ⚠️模拟 | +0.0% | 60 |
...
- **Elemental Conflux** (Lv25): 分别注入 74% MORE 到火/冰/电取平均。伤害构成：火 61.5% / 冰 33.6% / 电 4.9%
  三次模拟 DPS：火 84507 / 冰 76388 / 电 68046
...
- **构筑已有 modifier**：施法速度 INC 153%（来自 POB skillModList），总 MORE ×1.30。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +60.6% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
| 2 | Archmage（大法师） | 100 | +38.5% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
...
| 技能 | 效果描述 | DPS 预估 |
|------|----------|----------|
| **Pinnacle of Power** | 7 × (15% + 20×0.1%) = 119% MORE 元素伤害（PowerChargesMax） | **+93.2%** |
| **Elemental Conflux** | 元素伤害 MORE（期望 59%×1/3≈20%，等级20，简化模拟） | **+15.7%** |
...
当前 **Power Siphon** TotalDPS = **228,258**，AverageHit = 63,154，Speed = 3.61/s，CritChance = 85.3%，CritMultiplier = 4.62x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | elemental_pen | BASE | 8 | +14% | 需要 +14% 穿透 → DPS +18.9% |
| 2 | fire_pen | BASE | 0 | +24% | 需要 +24% 穿透 → DPS +19.9% |
| 3 | crit_multi_base | BASE | 100 | +26% | CritBase 100→126, 需要 +26 → DPS +20.1% |
| 4 | cast_speed_inc | INC | 153 | +50% | INC 153%→204%, 需要 +50 → DPS +20.2% |
| 5 | crit_multi_inc | INC | 262 | +94% | INC 262%→356%, 需要 +94 → DPS +20.1% |
...
1. **Dimensional Weakspot**: DPS +20.2%
2. **Primal Sundering**: DPS +16.2%
3. **Exposed to the Inferno**: DPS +14.9%
4. **Cremation**: DPS +14.9%
5. **Unquenchable Iron**: DPS +13.7%

**⚠️ 17 个无效天赋**: Invocated Echoes, Sacrificial Blood, The Spring Hare, Blood Transfusion, Echoing Thunder, Energise, Heavy Frost, Echoing Frost 等 17 个
