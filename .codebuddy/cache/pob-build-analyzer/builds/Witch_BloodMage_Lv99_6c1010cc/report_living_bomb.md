...
2. **fire_pen** (BASE): 每 1% 提升 1.34% DPS，需要 +14% 达到 +20% DPS
...
### Fire → Lightning Conversion/Gain = Fire → Lightning: 增益 5.0%
...
### Fire → Cold Conversion/Gain = Fire → Cold: 增益 35.0%
...
### Fire → Fire Self-Gain = Fire Self-Gain: 增益 65.0%
...
**公式**: `基础 4053-6123 × 效果 1.1100 × effMult 1.1200  (持续 3.56s, 几率 17.1%)`
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 火焰 +16.0% (占80.3%) | Enemy | +16.0 |
| 冰霜 +16.0% (占17.2%) | Enemy | +16.0 |
...
| 2 | fire_pen | BASE | +14.5% | % | 1.34%/% | 0 | 需要 +14% 穿透 → DPS +19.4% |
...
| 6 | spell_damage_inc | INC | +163.0% | % | 0.12%/% | 709 | INC 709%→872%, 需要 +163 → DPS +20.1% |
| 7 | elemental_damage_inc | INC | +163.0% | % | 0.12%/% | 709 | INC 709%→872%, 需要 +163 → DPS +20.1% |
| 8 | fire_damage_inc | INC | +202.0% | % | 0.10%/% | 709 | INC 709%→911%, 需要 +202 → DPS +20.1% |

**无影响维度**: physical_damage_inc, cold_damage_inc, lightning_damage_inc, chaos_damage_inc, crit_chance_inc, crit_chance_base, lightning_pen, cold_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Sunder the Flesh | Notable | -40.3% | +0.0% | 输出 | Base Critical Hit Chance for Spells is 15% |
| 2 | Stormbreaker | Notable | -9.9% | +0.0% | 输出 | 20% increased Damage for each type of Elemental Ailment on Enemy |
...
| 8 | Thin Ice | Notable | -6.2% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
...
| 10 | For the Jugular | Notable | -5.2% | -0.2% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 11 | Deadly Force | Notable | -5.2% | +0.0% | 输出 | 25% increased Damage if you've dealt a Critical Hit in the past 8 seconds; 10% increased Critical Hit Chance |
| 12 | Critical Overload | Notable | -5.0% | +0.0% | 输出 | 15% increased Critical Hit Chance for Spells; 15% increased Spell Damage if you've dealt a Critical Hit Recently |
| 13 | Sacrificial Blood | Notable | -4.9% | +0.0% | 输出 | 15% increased Life Cost of Skills; 40% increased Spell Damage with Spells that cost Life |
| 14 | Dynamism | Notable | -4.9% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
...
| 19 | Potent Incantation | Notable | -1.8% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Cremation | Notable | +28.5% | +0.0% | 输出 |
| 2 | Dimensional Weakspot | Notable | +25.9% | +0.0% | 输出 |
| 3 | Exposed to the Inferno | Notable | +24.9% | +0.0% | 输出 |
| 4 | Breath of Fire | Notable | +21.0% | +0.3% | 兼顾 |
| 5 | Smoke Inhalation | Notable | +20.8% | +0.0% | 输出 |
| 6 | Primal Sundering | Notable | +20.7% | +0.0% | 输出 |
| 7 | Burn Away | Notable | +15.6% | +0.0% | 输出 |
| 8 | Deep Freeze | Notable | +13.8% | +0.0% | 输出 |
| 9 | Cooked | Notable | +12.5% | -6.7% | 兼顾 |
| 10 | Barbaric Strength | Notable | +9.6% | +0.3% | 兼顾 |
...
- **DPS 贡献**: +9.9% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
- **分配天赋**: invocated limit, harness the elements (DPS +9.9%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | invocated limit | -0.0% | -0.0% |
| GrantedPassive | LIST | harness the elements | +9.9% | -0.0% |
...
| Total Resonance Count=300 | +81.0% | +86.2% | +5.2% | 131%→153% |
...
- **Elemental Conflux** (Lv25): 分别注入 74% MORE 到火/冰/电取平均。伤害构成：火 80.3% / 冰 17.2% / 电 2.5%
  三次模拟 DPS：火 49774 / 冰 35200 / 电 31782
...
当前 **Living Bomb** TotalDPS = **141,907**，AverageHit = 33,654，Speed = 4.22/s，CritChance = 85.3%，CritMultiplier = 4.62x。
...
| 2 | fire_pen | BASE | 0 | +14% | 需要 +14% 穿透 → DPS +19.4% |
...
1. **Cremation**: DPS +28.5%
2. **Dimensional Weakspot**: DPS +25.9%
3. **Exposed to the Inferno**: DPS +24.9%
4. **Breath of Fire**: DPS +21.0%，EHP +0.3%
5. **Smoke Inhalation**: DPS +20.8%
...
**最佳**: Megalomaniac (DPS +9.9%, EHP -0.0%)
