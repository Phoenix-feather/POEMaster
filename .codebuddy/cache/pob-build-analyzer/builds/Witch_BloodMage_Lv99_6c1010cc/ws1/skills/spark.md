...
2. **lightning_pen** (BASE): 每 1% 提升 0.87% DPS，需要 +22% 达到 +20% DPS
...
### 元素伤害 MORE (Lightning,Cold,Fire) = +457.6%
...
### Lightning → Lightning Self-Gain = Lightning Self-Gain: 增益 5.0%
...
### Lightning → Cold Conversion/Gain = Lightning → Cold: 增益 35.0%
...
### Lightning → Fire Conversion/Gain = Lightning → Fire: 增益 65.0%
...
**公式**: `基础 1223-23166 × 效果 1.1100 × effMult 1.1200  (持续 3.56s, 几率 34.2%)`

**类别汇总**: Sim: +2900.0 | Skill: +1900.0 | Tree: +185.0 | Jewel: +80.0 | Item: +11.0
...
### Combined DPS = 772,797

**类别汇总**: Hit: +750853.5 | DOT: +21943.5
...
### Cold-Infused 弹体数量 BASE = 18

**类别汇总**: SkillEffect: +18.0

| 来源 | 类别 | 值 |
|------|------|-----|
| Cold-Infused效果 | SkillEffect | +18.0 |
...
| 来源 | 类别 | 值 |
|------|------|-----|
| 闪电 +16.0% (占51.8%) | Enemy | +16.0 |
| 火焰 +16.0% (占31.2%) | Enemy | +16.0 |
| 冰霜 +16.0% (占17.0%) | Enemy | +16.0 |
...
| 2 | lightning_pen | BASE | +22.5% | % | 0.87%/% | 0 | 需要 +22% 穿透 → DPS +19.7% |
| 3 | crit_multi_base | BASE | +26.5% | % | 0.74%/% | 100 | CritBase 100→126, 需要 +26 → DPS +19.6% |
| 4 | fire_pen | BASE | +37.5% | % | 0.54%/% | 0 | 需要 +38% 穿透 → DPS +20.4% |
| 5 | cast_speed_inc | INC | +50.5% | % | 0.40%/% | 153 | INC 153%→204%, 需要 +50 → DPS +20.2% |
| 6 | crit_multi_inc | INC | +95.5% | % | 0.21%/% | 262 | INC 262%→358%, 需要 +96 → DPS +20.0% |
| 7 | spell_damage_inc | INC | +159.5% | % | 0.13%/% | 705 | INC 705%→864%, 需要 +160 → DPS +20.1% |
| 8 | projectile_damage_inc | INC | +159.5% | % | 0.13%/% | 705 | INC 705%→864%, 需要 +160 → DPS +20.1% |
| 9 | elemental_damage_inc | INC | +159.5% | % | 0.13%/% | 705 | INC 705%→864%, 需要 +160 → DPS +20.1% |
| 10 | lightning_damage_inc | INC | +311.0% | % | 0.06%/% | 705 | INC 705%→1016%, 需要 +311 → DPS +20.0% |

**无影响维度**: physical_damage_inc, fire_damage_inc, cold_damage_inc, chaos_damage_inc, crit_chance_inc, crit_chance_base, cold_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Sunder the Flesh | Notable | -30.2% | +0.0% | 输出 | Base Critical Hit Chance for Spells is 15% |
| 2 | Stormbreaker | Notable | -10.1% | +0.0% | 输出 | 20% increased Damage for each type of Elemental Ailment on Enemy |
| 3 | Throatseeker | Notable | -8.9% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 4 | Stormcharged | Notable | -8.1% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
...
| 7 | Sudden Escalation | Notable | -6.5% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 8 | Thin Ice | Notable | -6.3% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 9 | Evocational Practitioner | Notable | -5.4% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
...
| 11 | For the Jugular | Notable | -5.2% | -0.2% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 12 | Critical Overload | Notable | -5.1% | +0.0% | 输出 | 15% increased Critical Hit Chance for Spells; 15% increased Spell Damage if you've dealt a Critical Hit Recently |
| 13 | Sacrificial Blood | Notable | -5.0% | +0.0% | 输出 | 15% increased Life Cost of Skills; 40% increased Spell Damage with Spells that cost Life |
| 14 | Dynamism | Notable | -5.0% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 15 | True Strike | Notable | -4.3% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 16 | All Natural | Notable | -3.8% | -5.3% | 兼顾 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| 18 | Practiced Signs | Notable | -2.4% | +0.0% | 输出 | 6% increased Cast Speed |
| 19 | Potent Incantation | Notable | -1.9% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
...
| 2 | Primal Sundering | Notable | +20.7% | +0.0% | 输出 |
| 3 | Electric Amplification | Notable | +20.1% | +0.0% | 输出 |
| 4 | Exposed to the Storm | Notable | +16.1% | +0.0% | 输出 |
| 5 | Storm Surge | Notable | +15.8% | +0.0% | 输出 |
| 6 | Deep Freeze | Notable | +13.8% | +0.0% | 输出 |
| 7 | Cremation | Notable | +13.4% | +0.0% | 输出 |
| 8 | Breath of Lightning | Notable | +13.4% | +0.0% | 输出 |
| 9 | Flash Storm | Notable | +13.4% | +0.0% | 输出 |
| 10 | Surging Currents | Notable | +13.4% | +0.0% | 输出 |
...
- **DPS 贡献**: +10.1% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
- **分配天赋**: invocated limit, harness the elements (DPS +10.1%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | invocated limit | -0.0% | -0.0% |
| GrantedPassive | LIST | harness the elements | +10.1% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +66.0% | +70.7% (辅助+4.7%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +81.0% | +86.1% | +5.2% | 131%→153% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Lightning Mastery**: +1 level
- 总辅助贡献: **+4.7%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+5.2%
...
| 2 | Charge Infusion | +33.0% | +33.0% ⚠️模拟 | +0.0% | 30 |
...
- **Elemental Conflux** (Lv25): 分别注入 74% MORE 到火/冰/电取平均。伤害构成：火 31.2% / 冰 17.0% / 电 51.8%
  三次模拟 DPS：火 203308 / 冰 185938 / 电 228551
...
- **构筑已有 modifier**：施法速度 INC 153%（来自 POB skillModList），总 MORE ×1.56。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| 2 | Archmage（大法师） | 100 | +17.8% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Mysticism II | 30 | +5.0% | 需精魄 30（缺 30） ⚠️估算 | 动态扫描 |
...
- Charge Infusion 使用构筑实际等级 Lv24（效果值=26），非满级 Lv20
...
当前 **Spark** TotalDPS = **750,853**，AverageHit = 207,746，Speed = 3.61/s，CritChance = 85.3%，CritMultiplier = 4.62x。
...
| 2 | lightning_pen | BASE | 0 | +22% | 需要 +22% 穿透 → DPS +19.7% |
| 3 | crit_multi_base | BASE | 100 | +26% | CritBase 100→126, 需要 +26 → DPS +19.6% |
| 4 | fire_pen | BASE | 0 | +38% | 需要 +38% 穿透 → DPS +20.4% |
| 5 | cast_speed_inc | INC | 153 | +50% | INC 153%→204%, 需要 +50 → DPS +20.2% |
...
2. **Primal Sundering**: DPS +20.7%
3. **Electric Amplification**: DPS +20.1%
4. **Exposed to the Storm**: DPS +16.1%
5. **Storm Surge**: DPS +15.8%
...
**最佳**: Megalomaniac (DPS +10.1%, EHP -0.0%)
