...
5. 💡 **flat_lightning_attack** 对 DPS 影响最大（每 1 提升 12.82% DPS）

### 优化方向 Top 3

1. **flat_lightning_attack** (BASE): 每 1 提升 12.82% DPS，需要 +2 达到 +20% DPS
2. **flat_fire_attack** (BASE): 每 1 提升 12.33% DPS，需要 +2 达到 +20% DPS
3. **flat_cold_attack** (BASE): 每 1 提升 11.46% DPS，需要 +2 达到 +20% DPS
...
**类别汇总**: Skill: +72.0 | Sim: +26.0 | Tree: +16.0 | Item: +8.0
...
### CritChance BASE = 22.7% (POB实际, 手动推算=23.4%)
...
|---|------|------|--------|------|----------|--------|------|
| 1 | flat_lightning_attack | BASE | +2.0 |  | 12.82%/1 | 0 | 需要添加 2-4 基础伤害 → DPS +25.6% |
| 2 | flat_fire_attack | BASE | +2.0 |  | 12.33%/1 | 0 | 需要添加 2-4 基础伤害 → DPS +24.7% |
| 3 | flat_cold_attack | BASE | +2.0 |  | 11.46%/1 | 0 | 需要添加 2-4 基础伤害 → DPS +22.9% |
| 4 | crit_chance_base | BASE | +3.5% | % | 5.72%/% | 0 | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +20.0% |
| 5 | flat_physical_attack | BASE | +5.5 |  | 3.94%/1 | 0 | 需要添加 6-11 基础伤害 → DPS +21.7% |
| 6 | attack_speed_inc | INC | +44.5% | % | 0.46%/% | 122 | INC 122%→166%, 需要 +44 → DPS +20.3% |
| 7 | crit_multi_base | BASE | +45.5% | % | 0.45%/% | 100 | CritBase 100→146, 需要 +46 → DPS +20.4% |
| 8 | attack_damage_inc | INC | +72.0% | % | 0.28%/% | 190 | INC 190%→262%, 需要 +72 → DPS +20.1% |
| 9 | melee_damage_inc | INC | +72.0% | % | 0.28%/% | 190 | INC 190%→262%, 需要 +72 → DPS +20.1% |
| 10 | cold_damage_inc | INC | +98.5% | % | 0.20%/% | 190 | INC 190%→288%, 需要 +98 → DPS +20.2% |
| 11 | elemental_damage_inc | INC | +98.5% | % | 0.20%/% | 190 | INC 190%→288%, 需要 +98 → DPS +20.2% |
| 12 | crit_chance_inc | INC | +115.0% | % | 0.17%/% | 130 | INC 130%→245%, 需要 +115 → DPS +20.0% |
| 13 | crit_multi_inc | INC | +124.0% | % | 0.16%/% | 174 | INC 174%→298%, 需要 +124 → DPS +20.0% |
| 14 | physical_damage_inc | INC | +254.5% | % | 0.08%/% | 190 | INC 190%→444%, 需要 +254 → DPS +20.0% |
...
|---|------|------|-------------|-------------|------|------|
| 1 | Heavy Frost | Notable | -35.9% | +0.0% | 输出 | 20% increased Freeze Buildup; Hits ignore non-negative Elemental Resistances of Frozen Enemies |
| 2 | Thin Ice | Notable | -13.5% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 3 | Dynamism | Notable | -10.7% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 4 | Stormcharged | Notable | -10.4% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
| 5 | Throatseeker | Notable | -6.8% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 6 | Careful Assassin | Notable | -6.2% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 7 | Gore Spike | Notable | -5.8% | +0.0% | 输出 | 1% increased Critical Damage Bonus per 50 current Life |
| 8 | All Natural | Notable | -5.8% | +0.0% | 输出 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
| 9 | True Strike | Notable | -4.9% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 10 | Acceleration | Notable | -4.5% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 11 | Evocational Practitioner | Notable | -4.4% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
| 12 | For the Jugular | Notable | -4.0% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Innate Rune | Notable | +147.4% | +0.0% | 输出 |
| 2 | Avatar of Fire | Keystone | +113.2% | +0.0% | 输出 |
| 3 | Unquenchable Iron | Notable | +53.7% | +0.0% | 输出 |
| 4 | Chakra of Elements | Notable | +48.2% | +0.0% | 输出 |
| 5 | Burning Strikes | Notable | +35.1% | +0.0% | 输出 |
| 6 | Wild Storm | Notable | +27.2% | +0.0% | 输出 |
| 7 | Frozen Claw | Notable | +25.1% | +0.0% | 输出 |
| 8 | Electrified Claw | Notable | +23.2% | +0.0% | 输出 |
...
- **DPS 贡献**: +21.1% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 21984
- **分配天赋**: invocated limit, harness the elements (DPS +21.1%, EHP -0.0%)

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| GrantedPassive | LIST | invocated limit | -0.0% | -0.0% |
| GrantedPassive | LIST | harness the elements | +21.1% | -0.0% |

### Blight Wound (Sapphire, RARE)

- **DPS 贡献**: +0.9% | **EHP 贡献**: +3.5% | **状态**: ok | **槽位**: Jewel 7960
...
- **DPS 贡献**: -0.0% | **EHP 贡献**: +3.0% | **状态**: ok | **槽位**: Jewel 61834
...
- **DPS 贡献**: +1.9% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 32763

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| CritMultiplier | INC | 12 | +1.9% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +39.6% | +44.1% (辅助+4.5%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
|------|--------|------|----------|-----------|
| Total Resonance Count=0 | +0.0% | +0.0% | — | 100% |
| Total Resonance Count=300 | +47.8% | +52.6% | +4.8% | 100%→122% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Lightning Mastery**: +1 level
- 总辅助贡献: **+4.5%** DPS（Total Resonance Count=150时）
...
| 2 | Charge Infusion | +23.5% | +23.5% ⚠️模拟 | +0.0% | 30 |
...
| 3 | Elemental Conflux | +9.8% | +9.8% ⚠️模拟 | +0.0% | 60 |
...
- **构筑已有 modifier**：施法速度 INC 122%（来自 POB skillModList），总 MORE ×1.00。INC 叠加为加法（新增边际递减），MORE 叠加为乘法
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Attrition（损耗） | 30 | +63.9% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
...
| # | 精魄辅助 | 精魄 | DPS% | 条件 | 来源 |
|---|----------|------|------|------|------|
| 1 | Direstrike II（猛击 II） | 40 | +19.5% | 需精魄 40（缺 40）; Low Life | 硬编码 |
| 2 | Direstrike I（猛击 I） | 20 | +14.1% | 需精魄 20（缺 20）; Low Life | 硬编码 |
| 3 | Precision I（精准 I） | 10 | +4.4% | 需精魄 10（缺 10）; 仅攻击构筑 | 硬编码 |
| 4 | Precision II（精准 II） | 20 | +4.4% | 需精魄 20（缺 20）; 仅攻击构筑 | 硬编码 |
...
| 技能 | 效果描述 | DPS 预估 |
|------|----------|----------|
| **Pinnacle of Power** | 4 × (15% + 20×0.1%) = 68% MORE 元素伤害（PowerChargesMax） | **+44.1%** |
...
当前 **Shred** TotalDPS = **5,371**，AverageHit = 0，Speed = 5.82/s，CritChance = 22.7%，CritMultiplier = 3.74x。
...
| # | 维度 | 类型 | 当前值 | 所需值 | 公式 |
|---|------|------|--------|--------|------|
| 1 | flat_lightning_attack | BASE | 0 | +2 | 需要添加 2-4 基础伤害 → DPS +25.6% |
| 2 | flat_fire_attack | BASE | 0 | +2 | 需要添加 2-4 基础伤害 → DPS +24.7% |
| 3 | flat_cold_attack | BASE | 0 | +2 | 需要添加 2-4 基础伤害 → DPS +22.9% |
| 4 | crit_chance_base | BASE | 0 | +4% | baseCrit 0.0%→3.5%, 需要 +3.5% → DPS +20.0% |
| 5 | flat_physical_attack | BASE | 0 | +6 | 需要添加 6-11 基础伤害 → DPS +21.7% |
...
1. **Innate Rune**: DPS +147.4%
2. **Avatar of Fire**: DPS +113.2%
3. **Unquenchable Iron**: DPS +53.7%
4. **Chakra of Elements**: DPS +48.2%
5. **Burning Strikes**: DPS +35.1%
...
**最佳**: Megalomaniac (DPS +21.1%, EHP -0.0%)
