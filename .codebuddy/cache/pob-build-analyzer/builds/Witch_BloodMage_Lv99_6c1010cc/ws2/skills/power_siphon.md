...
| 来源 | 类别 | 值 |
|------|------|-----|
| 冰霜 +16.0% (占85.0%) | Enemy | +16.0 |
| 物理 +0.0% (占15.0%) | Enemy | +0.0 |
...
| 3 | crit_multi_inc | INC | +80.0% | % | 0.25%/% | 204 | INC 204%→284%, 需要 +80 → DPS +20.1% |
| 4 | spell_damage_inc | INC | +92.5% | % | 0.22%/% | 453 | INC 453%→546%, 需要 +92 → DPS +20.0% |
| 5 | cold_damage_inc | INC | +164.0% | % | 0.12%/% | 453 | INC 453%→617%, 需要 +164 → DPS +20.0% |
| 6 | elemental_damage_inc | INC | +164.0% | % | 0.12%/% | 453 | INC 453%→617%, 需要 +164 → DPS +20.0% |
| 7 | physical_damage_inc | INC | +210.0% | % | 0.10%/% | 322 | INC 322%→532%, 需要 +210 → DPS +20.0% |

**无影响维度**: fire_damage_inc, lightning_damage_inc, chaos_damage_inc, crit_chance_inc, crit_chance_base, lightning_pen, fire_pen, cold_pen, elemental_pen, chaos_pen, projectile_count, aoe_inc, duration_inc
...
|---|------|------|-------------|-------------|------|------|
| 1 | Heavy Frost | Notable | -28.3% | +0.0% | 输出 | 20% increased Freeze Buildup; Hits ignore non-negative Elemental Resistances of Frozen Enemies |
| 2 | Throatseeker | Notable | -11.3% | +0.0% | 输出 | 60% increased Critical Damage Bonus; 20% reduced Critical Hit Chance |
| 3 | Thin Ice | Notable | -10.8% | +0.0% | 输出 | 20% increased Freeze Buildup; 50% increased Damage with Hits against Frozen Enemies |
| 4 | Gore Spike | Notable | -9.2% | +0.0% | 输出 | 1% increased Critical Damage Bonus per 50 current Life |
| 5 | Dynamism | Notable | -8.6% | +0.0% | 输出 | 40% increased Damage if you've Triggered a Skill Recently; Meta Skills gain 15% increased Energy |
| 6 | Stormcharged | Notable | -8.2% | +0.0% | 输出 | 40% increased Elemental Damage if you've dealt a Critical Hit Recently; 15% increased Critical Hit Chance |
| 7 | Careful Assassin | Notable | -7.2% | +0.0% | 输出 | 20% reduced Critical Damage Bonus; 50% increased Critical Hit Chance |
| 8 | Sudden Escalation | Notable | -6.7% | +0.0% | 输出 | 16% increased Critical Hit Chance for Spells; 8% increased Cast Speed if you've dealt a Critical Hit Recently |
| 9 | For the Jugular | Notable | -6.3% | -0.3% | 兼顾 | 25% increased Critical Damage Bonus; +10 to Intelligence |
| 10 | Evocational Practitioner | Notable | -5.7% | +0.0% | 输出 | 25% increased Critical Hit Chance if you've Triggered a Skill Recently; Meta Skills gain 25% increased Energy if you've dealt a Critical Hit Recently |
| 11 | Potent Incantation | Notable | -4.7% | +0.0% | 输出 | 30% increased Spell Damage; 5% reduced Cast Speed |
| 12 | True Strike | Notable | -4.6% | +0.0% | 输出 | +10 to Dexterity; 20% increased Critical Hit Chance |
| 13 | Acceleration | Notable | -3.9% | +0.0% | 输出 | 3% increased Movement Speed; 10% increased Skill Speed |
| 14 | All Natural | Notable | -3.7% | +0.0% | 输出 | +5% to all Elemental Resistances; 30% increased Elemental Damage |
...
| 16 | Practiced Signs | Notable | -2.3% | +0.0% | 输出 | 6% increased Cast Speed |
| 17 | The Spring Hare | Notable | -2.1% | +0.0% | 输出 | 20% chance for Damage of Enemies Hitting you to be Unlucky; 20% chance for Damage with Hits to be Lucky |
...
| # | 天赋 | 类型 | DPS% | EHP% | 分类 |
|---|------|------|------|------|------|
| 1 | Avatar of Fire | Keystone | +66.5% | +0.0% | 输出 |
| 2 | Unquenchable Iron | Notable | +41.4% | +0.0% | 输出 |
| 3 | Chakra of Elements | Notable | +36.7% | +0.0% | 输出 |
| 4 | Burning Strikes | Notable | +26.6% | +0.0% | 输出 |
| 5 | Wild Storm | Notable | +19.2% | +0.0% | 输出 |
| 6 | Volcanic Skin | Notable | +17.8% | +0.0% | 输出 |
| 7 | Cooked | Notable | +15.0% | -3.9% | 兼顾 |
| 8 | Essence of the Storm | Notable | +11.8% | +0.0% | 输出 |
| 9 | Essence of the Mountain | Notable | +11.5% | +0.0% | 输出 |
| 10 | Molten Being | Notable | +11.3% | +1.6% | 兼顾 |
...
- **DPS 贡献**: +4.8% | **EHP 贡献**: +2.0% | **状态**: ok | **槽位**: Jewel 61834

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| Damage | INC | 12 | +2.6% | -0.0% |
| EnergyShield | INC | 17 | -0.0% | +2.0% |
| CritChance | INC | 10 | +2.3% | -0.0% |

### Blight Wound (Sapphire, RARE)

- **DPS 贡献**: +4.1% | **EHP 贡献**: +2.3% | **状态**: ok | **槽位**: Jewel 7960

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| EnergyShield | INC | 20 | -0.0% | +2.3% |
| CritChance | INC | 14 | +3.2% | -0.0% |
| AilmentMagnitude | INC | 11 | +0.9% | -0.0% |

### Heart of the Well (Diamond, UNIQUE)

- **DPS 贡献**: +3.0% | **EHP 贡献**: -0.0% | **状态**: ok | **槽位**: Jewel 32763

| Mod | 类型 | 值 | DPS% | EHP% |
|-----|------|-----|------|------|
| CritMultiplier | INC | 12 | +3.0% | -0.0% |
...
|---|------|------------|----------|-----|------|
| 1 | Trinity | +40.3% | +44.3% (辅助+3.9%) (条件: Total Resonance Count=150) | +0.0% | 100 |
...
| Total Resonance Count=300 | +48.9% | +53.0% | +4.2% | 135%→157% |

### 辅助贡献

- **Dialla's Desire**: +1 level, +10% quality
- **Lightning Mastery**: +1 level
- 总辅助贡献: **+3.9%** DPS（Total Resonance Count=150时）
- 端点差异: Total Resonance Count=0时+0.0%，Total Resonance Count=300时+4.2%
...
| 2 | Charge Infusion | +33.4% | +33.4% ⚠️模拟 | +0.0% | 30 |
...
| 3 | Elemental Conflux | +11.5% | +11.5% ⚠️模拟 | +0.0% | 60 |
...
- **Elemental Conflux** (Lv25): 分别注入 74% MORE 到火/冰/电取平均。伤害构成：火 0.0% / 冰 100.0% / 电 0.0%
  三次模拟 DPS：火 16580 / 冰 22289 / 电 16580
...
| # | 光环 | 精魄 | DPS% | EHP% | 说明 |
|---|------|------|------|------|------|
| 1 | Archmage（大法师） | 100 | +74.3% | +0.0% | 需精魄 100（缺 100）; Mana 转附加闪电伤害 |
| 2 | Attrition（损耗） | 30 | +63.8% | +0.0% | 需精魄 30（缺 30）; 命中附带 Wither 叠层 |
...
| 2 | Mysticism I | 15 | +6.5% | 需精魄 15（缺 15） ⚠️估算 | 动态扫描 |
...
| 技能 | 效果描述 | DPS 预估 |
|------|----------|----------|
| **Pinnacle of Power** | 4 × (15% + 20×0.1%) = 68% MORE 元素伤害（PowerChargesMax） | **+45.9%** |
| **Elemental Conflux** | 元素伤害 MORE（期望 59%×1/3≈20%，等级20，简化模拟） | **+13.5%** |
...
当前 **Power Siphon** TotalDPS = **51,192**，AverageHit = 13,943，Speed = 3.67/s，CritChance = 79.2%，CritMultiplier = 4.04x。
...
| 3 | crit_multi_inc | INC | 204 | +80% | INC 204%→284%, 需要 +80 → DPS +20.1% |
| 4 | spell_damage_inc | INC | 453 | +92% | INC 453%→546%, 需要 +92 → DPS +20.0% |
| 5 | cold_damage_inc | INC | 453 | +164% | INC 453%→617%, 需要 +164 → DPS +20.0% |
...
1. **Avatar of Fire**: DPS +66.5%
2. **Unquenchable Iron**: DPS +41.4%
3. **Chakra of Elements**: DPS +36.7%
4. **Burning Strikes**: DPS +26.6%
5. **Wild Storm**: DPS +19.2%
